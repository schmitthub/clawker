...
3. **Subsystem failures degrade, never cascade.** A broken init Executor → `initExec = nil`; entrypoint fifo timeout becomes the user-visible failure; the firewall, registry, AdminService, dialer all stay up. A broken dialer → `dialer = nil`; CP→clawkerd dispatch disabled; everything else stays up. The pattern at `cmd/clawker-cp/main.go:718-721` (dialer) and `:707-714` (initExec) is the template — copy it for any new subsystem.
