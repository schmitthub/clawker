...
│   │   ├── agentregistry/     # SQLite-persisted identity store — CLI writes `(thumbprint, container_id, canonical_cn)` rows at container create time; CP-side reaper + dockerevents `ContainerDestroyed`/`ContainerRemoved` evict
...
│   │   ├── dockerevents/      # Docker events feeder + reconcile + 1:1 moby-action typed event publishers (`ContainerCreated/Started/Restarted/Paused/Unpaused/Died/Stopped/Killed/OOM/Destroyed/Removed/Renamed`, `NetworkCreated/Connected/Disconnected/Destroyed`). Each event embeds the moby `events.Message` verbatim — no lossy bucketing.
