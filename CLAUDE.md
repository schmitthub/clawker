...
│   │   ├── agentregistry/     # SQLite-persisted identity store — CLI writes `(thumbprint, container_id, canonical_cn)` rows at container create time; CP-side reaper + dockerevents `ContainerDestroyed` evict
...
│   │   ├── dockerevents/      # Docker events feeder + reconcile + 1:1 moby-action typed event publishers (`ContainerCreated/Started/Restarted/Paused/Unpaused/Died/Stopped/Killed/OOM/Destroyed/Renamed`, `NetworkCreated/Connected/Disconnected/Destroyed`). Each event embeds the moby `events.Message` verbatim — no lossy bucketing. moby fires `container/destroy` for `docker rm`; `ActionRemove` is image-only and never produces a container event.
