...
| `datapoint_renames` | Datapoint attribute copies — each `from`/`to` pair copies a datapoint attribute to a new key. |
...
Everything an extension ships except collector routing is applied as an
idempotent update against the running stack; collector routing is regenerated and
the collector recreated only when the rendered configuration changes. A project
selects the extension in `monitor.extensions`, and `clawker monitor up` seeds it.
There is no host-side registry — the active set is a projection of the project
you ran `up` from, and the collector routes from the union of every extension
ever seeded. See [Monitoring Extensions](/monitoring-extensions).
