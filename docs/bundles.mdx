...
the URL. A remote source requires either `ref` (a branch or tag) or `sha` (a
full 40-character commit); when both are set, `sha` wins and pins a reproducible
fetch. A `path:`-only entry loads a local directory in place with no cache copy,
...
A `ref` (branch or tag) source drifts only when you ask it to. `clawker bundle
update` compares the declared source against its current tip and refetches only
on a change:
