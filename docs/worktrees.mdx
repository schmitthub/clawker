...
Container and loop commands accept a `--worktree` flag that automatically creates or reuses a worktree for the container's `/workspace` mount:
...
If the worktree exists in the registry but is unhealthy (directory deleted, Git metadata missing, or branch deleted), the command fails with a message suggesting `clawker worktree prune` to clean up the stale entry first.
