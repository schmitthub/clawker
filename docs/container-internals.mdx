...
Volumes are named `clawker.<project>.<agent>-<name>` (e.g. `clawker.myapp.dev-config`). Because these are Docker volumes, they survive container removal and recreation. As long as you use the same project and agent name, your harness state — including the auth token from your in-container login — is preserved.
...
| Harness config volume(s) | `clawker.<project>.<agent>-<name>` (one per bundle-declared dir) | `clawker.myapp.dev-config` |
| History volume | `clawker.<project>.<agent>-history` | `clawker.myapp.dev-history` |
| Lifecycle volume | `clawker.<project>.<agent>-clawker` | `clawker.myapp.dev-clawker` |
| Workspace volume | `clawker.<project>.<agent>-workspace` | `clawker.myapp.dev-workspace` (snapshot mode only) |

All resources are tagged with labels (`dev.clawker.project`, `dev.clawker.agent`; containers and images also carry `dev.clawker.harness`) for filtering and management. The `clawker container ls` command uses these labels to show only Clawker-managed containers.
