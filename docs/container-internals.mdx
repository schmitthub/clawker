...
When you run `clawker run @` or `clawker container create @`, the CLI first ensures the control plane is fully up and its clock is in sync with the host — the bootstrap material is minted against CP's clock, so creation will not proceed against an unsynced one (see [control plane clock sync](/control-plane#troubleshooting) if creation fails this check). The container then goes through four host-side phases before clawkerd takes over inside:
