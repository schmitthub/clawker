...
- **Git config** — Your `~/.gitconfig` is bind-mounted read-only to a staging path inside the container. The entrypoint copies it to `~/.gitconfig` while filtering out `[credential]` sections (since the container uses its own credential forwarding).
