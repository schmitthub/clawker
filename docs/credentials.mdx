...
3. The resulting token is written to the harness's config directory (e.g. `~/.claude`, `~/.codex`), which is backed by a named Docker volume (`clawker.<project>.<agent>-config`)
4. The token persists across container restarts **and recreates** — any container reusing the same project + agent name reuses the volume and stays authenticated
