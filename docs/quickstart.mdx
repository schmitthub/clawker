...
## 2. Initialize Clawker

```bash
clawker init
```

This creates the Clawker directory structure following the XDG spec (`~/.config/clawker/` for settings, `~/.local/share/clawker/` for data, `~/.local/state/clawker/` for logs). It also asks if you want to build a default fallback image — this is optional but useful as a sandbox for quick experiments outside of a project.
...
- Creates a `.clawker.yaml` config file with sensible defaults
- Creates a `.clawkerignore` file (like `.dockerignore` for snapshot mode)
- Registers your project in Clawker's registry so it always knows the project root

<Tip>
The generated `.clawker.yaml` template includes GitHub domains and an SSH rule as a starting point. If your project needs npm, PyPI, or other services, add their domains to `security.firewall.add_domains`. See the [Firewall guide](/firewall#common-setups) for examples.
</Tip>
...
## 7. Customize Your Config

Edit `.clawker.yaml` to tailor the environment. Here's a typical setup for a Node.js project:
...
## 8. Parallel Agents with Worktrees
...
## 10. Autonomous Loops (Experimental)
