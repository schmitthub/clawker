...
Open OpenSearch Dashboards at `http://localhost:5601` to explore logs (costs, token usage, tool calls, session events, firewall egress, DNS queries), and Prometheus at `http://localhost:9090` for metrics. The stack ships bare — first-run setup requires creating a Dashboards index pattern covering `clawker-*,claude-code` (the four indices: `claude-code`, `clawker-cp`, `clawker-envoy`, `clawker-coredns`). See [Monitoring](/monitoring) for the full setup checklist.
