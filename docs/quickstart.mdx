...
This creates:
- A `.clawker.yaml` config file pre-filled with the right packages, language toolchains, and firewall domains for your language
...
This builds the image for your default harness. Two stages come out of it: a shared base image (`clawker-<project>:base`) holding the harness-agnostic layers from your `.clawker.yaml` — system packages, language toolchains, custom instructions — and a harness image layered on top, tagged with the harness name (e.g. `clawker-<project>:claude`). The default harness image also gets the `:default` alias. To build a specific registered harness instead, pass its name: `clawker build -t codex`. Docker's layer cache is used, so unchanged layers are skipped; use `--no-cache` to force a full rebuild.
...
- `--dangerously-skip-permissions` — the infamous Claude Code yolo flag. Anything after the `@` is passed to the harness CLI as normal arguments, so you can treat it as a regular `claude` invocation, including `-c` to continue your previous session. You can also pass arguments after an alias, like `clawker go dev -c`.
...
1. Creates the container with your project directory mounted at its real host path, plus persistent volumes declared by the harness bundle — its config volume (`~/.claude` for the claude harness, `~/.codex` for codex) and shell history — and streams the container's mTLS bootstrap material (per-agent cert + key + CA + a single-use Hydra JWT, minted in your host's clock) into it
...
<CardGroup cols={2}>
  ...
  <Card title="Custom Images" icon="layer-group" href="/custom-images">
    Packages, language toolchains, Dockerfile instructions, and injection points
  </Card>
