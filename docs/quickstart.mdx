...
Open OpenSearch Dashboards at `http://localhost:5601` to explore logs (costs, token usage, tool calls, session events), and Prometheus at `http://localhost:9090` for metrics. The stack ships bare — first-run setup requires creating a Dashboards index pattern covering `claude-code` and `clawker-cp`. See [Monitoring](/monitoring) for the full setup checklist.
