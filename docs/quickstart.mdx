...
Anything after the `@` is passed to the harness CLI as normal arguments, and arguments after an alias are appended — so `clawker go dev -c` continues your previous Claude Code session, and `clawker go dev --dangerously-skip-permissions` hands Claude Code its infamous yolo flag, safe here because the agent is boxed in.
