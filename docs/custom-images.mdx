...
Stacks are self-guarding — each skips itself when the image already provides the runtime. Harness bundles declare their own stack requirements (for example, the `claude` bundle declares `node`); a harness-declared stack renders in the harness image with its own lineage-resolved definition, always — even when your project also declared the same name in the base. Both strata render; the fragment self-guards handle any overlap.

Stack definitions are file-backed (`stack.yaml` plus root- and/or user-scope Dockerfile fragments). Shipped definitions resolve straight from the clawker binary; register your own per-project in `clawker.yaml` under `stacks:` (or with `clawker stack register`). A closer layer — the project registry, then a harness bundle's own `stacks/` dir — shadows a shipped definition of the same name, reported in the build output. See [Stacks](/stacks) for the full definition format and lookup chain.
...
The base `build` block applies to every image you build. To layer extra stacks, packages, or inject steps onto **one** harness's image without touching the bundle or the base, declare them under `build.harnesses.<name>` — the same `stacks`, `packages`, and `inject` primitives, scoped to that harness's lineage:
...
build:
  ...
  harnesses:
    claude:
      stacks:
        - bun                 # claude image only, after the bundle's own stacks
      packages:
...
- **`stacks`** render in that harness's image **after** the bundle's own stacks (installer → overlay, declaration order). A name that appears in both the bundle and the overlay renders once, at its installer position.
...
An overlay keyed to a harness that resolves nowhere (not shipped, not registered) is a build error naming the `clawker harness register` remedy — a typo'd overlay never silently does nothing.
