...
The harness image adds whatever the selected harness bundle declares — for example, the `claude` bundle installs Claude Code (plus the `node` stack it runs on), while the `codex` bundle installs the standalone Codex binary — and ends with `ENTRYPOINT ["clawkerd"]` and the harness's `CMD`. See [Harnesses](/harnesses) for selecting and registering harnesses, and [Authoring Harnesses](/authoring-harnesses) to write your own bundle.
...
Clawker ships `go`, `node`, `python`, and `rust` stack definitions. See [Stacks](/stacks) for what each installs.

Stacks are self-guarding — each skips itself when the image already provides the runtime. A project-declared stack renders in the shared base before your `root_run`/`user_run`; a harness-declared stack (for example, the `claude` bundle declares `node`) renders in the harness image. Both strata render even when they share a name — the fragment self-guards handle any overlap.

See [Stacks](/stacks) for the full definition format and lookup chain.
