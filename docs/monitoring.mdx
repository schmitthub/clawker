...
**Dashboards.** Three dashboards are preinstalled:
...
- **Saved objects** imported INTO the Clawker workspace — index patterns for every log index and the three dashboards (`Clawker Networking`, `Claude Code Cost & Usage`, `Claude Code Activity`). Build additional dashboards off the index patterns + Prometheus datasource as needed.
