...
Fresh projects default to `[claude-code]`, so Claude Code telemetry is captured
out of the box. Each `clawker monitor up` idempotently seeds the extensions the
current project selects onto the running stack — there is no host-side registry,
and the collector routes from the union of every extension ever seeded. See
...
built-in `claude-code` extension (selected by default):
