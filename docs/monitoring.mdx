...
- **CoreDNS query logs** (`clawker-coredns`) — queried domain, qtype (A/AAAA), rcode (NOERROR / NXDOMAIN), resolution duration. Query `rcode:NXDOMAIN` for DNS-layer blocks; `rcode:NXDOMAIN` for allowed-zone upstream misses (typo, missing record). CoreDNS makes no explicit allow/deny decision per query — `rcode` is the only verdict signal.
