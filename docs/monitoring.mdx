...
Open OpenSearch Dashboards at `http://localhost:5601`. From the Discover view, create an index pattern covering `claude-code,clawker-cli,clawker-cp,clawker-envoy,clawker-coredns` to see Claude Code logs, host CLI logs, clawker-cp logs, and egress traffic (Envoy access + CoreDNS query logs) side-by-side. Metrics live at the Prometheus UI on `http://localhost:9090`.
