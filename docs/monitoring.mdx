...
| **OpenSearch** | `opensearchproject/opensearch` | 9200 | Stores logs (index `clawker-logs`) and traces (Simple Schema for Observability dataset `traces` / namespace `clawker`) |
...
When the firewall is enabled, Envoy and CoreDNS emit structured access logs to stdout. The OTEL collector picks them up (via the same Docker label discovery used for agents) and ships them into the `clawker-logs` index in OpenSearch alongside agent telemetry. From OpenSearch Dashboards you can build saved searches and visualizations over:
...
Open OpenSearch Dashboards at `http://localhost:5601`. From the Discover view, point at the `clawker-logs` index for agent + firewall logs, or use the Observability plugin's Trace Analytics view to inspect spans. Metrics live at the Prometheus UI on `http://localhost:9090`.
