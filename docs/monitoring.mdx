...
| **OpenSearch** | `opensearchproject/opensearch` | 9200 | Stores logs across five indices: `claude-code` (Claude Code OTLP), `clawker-cli` (host CLI records exported by the collector), `clawker-cp` (clawker-cp mTLS push), `clawker-envoy` (firewall HTTP/TLS/TCP access logs), `clawker-coredns` (firewall DNS query logs) |
...
Open OpenSearch Dashboards at `http://localhost:5601`. From the Discover view, pick one of the preconfigured index patterns — `claude-code`, `clawker-cli`, `clawker-cp`, `clawker-envoy`, or `clawker-coredns` — to see Claude Code logs, host CLI logs, clawker-cp logs, and egress traffic (Envoy access + CoreDNS query logs) side-by-side. Metrics live at the Prometheus UI on `http://localhost:9090`.
...
- **Index templates** with per-source field mappings for each of the five indices (so dynamic mapping never locks the wrong type at first ingest)
- **ISM retention policy** (7-day rollover-to-delete, auto-attached to all five indices via `ism_template.index_patterns`)
- **Dashboards saved objects** — index patterns for all five indices with `@timestamp` as the time field
...
<Note>
Curated dashboards, visualizations, alerts, and Prometheus-as-a-Dashboards-data-source are still TODO. The bootstrap surface is in place — adding pre-built dashboards is a matter of dropping their NDJSON export into `internal/monitor/templates/opensearch-bootstrap/saved-objects/clawker.ndjson` and shipping a new release.
</Note>
