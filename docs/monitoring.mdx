...
capture Claude Code telemetry. Each `clawker monitor up` idempotently seeds the extensions the
current project selects onto the running stack — there is no host-side registry,
and the collector routes from the union of every extension ever seeded. `up`
never restarts a running collector; to apply an extension edit to a running
stack, run `clawker monitor reload` (re-renders the config and recreates the
collector). See
