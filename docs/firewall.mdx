...
<Note>
This page is the reference for the firewall itself — architecture, configuration, and CLI commands. For *why* the firewall is the center of clawker's defenses and the specific attacks it neutralizes (prompt injection, data exfiltration, DNS tunneling), see the [Threat Model](/threat-model).
</Note>
...
3. Project rules from `.clawker.yaml` are merged with the selected harness's egress floor rules (additive merge, dedup by `destination:protocol:port`).
...
6. As the agent resolves DNS, the `dnsbpf` plugin writes `IP → {identity, TTL}` entries into the `dns_cache` map, where the identity is a per-destination route identity allocated by the control plane. On each outbound TCP connection, the `connect4` / `connect6` programs look up the destination IP in `dns_cache`, then look up `{identity, dst_port}` in `route_map` to decide which Envoy listener to redirect to.
...
Per-domain TCP routing depends on the `dns_cache` map staying consistent with the IPs a container actually dials, and the `dnsbpf` plugin keeps it consistent by tracking every query **live** rather than seeding the cache once at startup. A one-time seed would go stale as CDNs rotate backends behind a domain — silently blackholing traffic to an allowlisted domain until the next firewall reload. Because the plugin observes whatever the upstream resolver returns on each query, any IP a container ends up using has already been observed and hashed at the moment of connection.
...
The default allowlist is the selected harness's **egress floor** — each harness bundle declares the bare-minimum set of domains that harness needs (API access, authentication, telemetry), and clawker composes it with your project rules. The floor is deliberately bare-bones.

The **claude** harness floor:
...
| `registry.npmjs.org` | npm (Claude Code is npm-distributed; its runtime hooks install global packages) |
...
| Domain | Purpose |
|--------|---------|
| `api.openai.com` | API-key auth inference |
| `auth.openai.com` | Login/token exchange |
| `chatgpt.com` | ChatGPT-account backend, path-scoped: only the codex backend prefixes (`/backend-api/codex/`, `/backend-api/wham/usage`, `/backend-api/ps/`, `/backend-api/plugins/`, `/backend-api/connectors/directory/`) are allowed and every other path is denied — `chatgpt.com` also hosts publicly readable shared conversations, an exfiltration sink if the whole domain were open. A companion `wss` rule enables WebSocket upgrades on the same allowed paths for streaming responses |

Custom harness bundles declare their own floor in the manifest's `egress:` list, using the same rule shape as project `security.firewall.rules`.
...
<Warning>
**GitHub, PyPI, and most other services are NOT in any harness floor.** (The claude floor includes the npm registry because Claude Code itself is npm-distributed.) When you run `clawker project init`, the generated `.clawker.yaml` template includes `github.com` and `api.github.com` as a convenience starting point — but these are project-level config entries, not system defaults. If you start from a blank config, GitHub SSH and HTTPS will be blocked.
...
security:
  firewall:
    add_domains:
      # Node.js / npm — registry.npmjs.org is already in the claude harness floor
...
A typical project that uses GitHub and npm (the npm registry is already in the claude harness floor):
...
- **Harness floor rules** come from the selected harness bundle's `egress:` list and are always present --- the domains that harness needs for API access, authentication, and telemetry
- **Project rules** come from `add_domains` and `rules` in your `.clawker.yaml`
- Rules merge **additively** --- project rules add to (never replace) the harness floor
- Dedup key: `destination:protocol:port` --- duplicate rules are silently ignored
- This means you cannot accidentally override or remove a harness floor rule
...
| `clawker firewall prune` | Reset the rule store to what config defines: remove every rule, then re-sync the harness floor + project rules. The store is shared across projects — other projects' rules are removed too and come back on their next `refresh` or container start (`--all` removes everything and re-syncs nothing; prompts unless `--yes`) |
...
- Wildcard HTTPS/WSS rules obtain a certificate for the exact requested hostname from the control plane over mTLS. Names with multiple subdomain labels pass hostname verification with the installed firewall CA.
- Envoy's SDS client key has owner-only file permissions and is mounted read-only. CP sets the file owner and permissions before each atomic replacement.
- HTTP/3 uses static wildcard certificates. Clients need TCP fallback for names with more than one label below the wildcard zone. Exact-domain, IP, and CIDR rules retain static certificates.
...
- The control plane prevents CA reads during rotation. An SDS request only reads an existing CA; it cannot create a second CA during replacement.
...
Disabling the firewall also stops netlogger emission. The eBPF cgroup programs that produce egress decision records only run while the firewall is enabled, so the `clawker-ebpf-egress` OpenSearch index receives no new records while the firewall is off. See [Egress Observability](/observability) for the record shape.
