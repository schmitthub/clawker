...
not guess. An invalid path --- a literal missing the leading `/`, or a regex that
won't compile --- **fails the whole add/refresh operation** with an error rather
than silently loosening the rule.
