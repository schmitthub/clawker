...
| `clawker firewall prune` | Reset the rule store to what config defines: remove every rule, then re-sync the harness floor + project rules (`--all` removes everything and re-syncs nothing; prompts unless `--yes`) |
