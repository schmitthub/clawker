...
Both containers run on the `clawker-net` bridge network with static IPs: Envoy at `.200`, CoreDNS at `.201`. Agent containers join the same network, use CoreDNS for DNS resolution, and route HTTPS through Envoy via iptables DNAT rules configured during container init.
...
<Note>
Global firewall enablement is controlled in `settings.yaml` via `firewall.enable`, not in the project config.
</Note>
...
| `proto` | string | `tls`, `tcp`, or `ssh` (default: `tls`) |
| `port` | int | Destination port (default: 443 for TLS, 22 for SSH) |
| `action` | string | `allow` or `deny` (default: `allow`) |
| `path_rules` | list | Optional path patterns for TLS MITM inspection (each entry has `path` and `action`) |
| `path_default` | string | Default action for paths not matching any `path_rules` entry |

When `path_rules` is specified, Envoy performs TLS interception (MITM) for that domain --- it terminates the TLS connection, inspects the HTTP path, and only forwards requests matching the path patterns. Without `path_rules`, the domain uses SNI passthrough (no inspection).
