...
| `clawker firewall add DOMAIN` | Add a domain allow rule (use `--path` + `--action` to attach a path-scoped rule) |
| `clawker firewall remove DOMAIN` | Remove a domain rule (use `--path` to remove only that path rule, leaving the entry) |
...
Path rules accumulate across calls. Repeating a `--path` with a different `--action` overwrites that path's action. Use `clawker firewall remove api.example.com --path /v1` to drop a single path rule without removing the whole entry.
