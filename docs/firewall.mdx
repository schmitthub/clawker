...
<Warning>
**TCP/SSH rules capture all traffic on the port, not just traffic to the whitelisted domain.** Unlike TLS rules where Envoy can inspect the SNI to determine the intended destination, raw TCP and SSH connections have no domain-level metadata. Clawker handles this by redirecting **all** outbound traffic on the configured port (via iptables DNAT) to Envoy, which forwards it to the whitelisted domain's resolved IP.
...
This also means **two different domains cannot be whitelisted on the same non-TLS port**. If you need SSH access to both GitHub and GitLab, one of them would need to use a non-standard port, or you can configure one provider to use HTTPS git instead.
