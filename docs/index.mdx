...
In Clawker, an **agent** is a named container instance running a coding-agent harness (Claude Code, Codex, or a harness you add yourself). Each agent gets its own isolated environment with its own filesystem, network, and credentials. You can run multiple agents per project (e.g., `--agent dev`, `--agent review`) and they operate independently.
