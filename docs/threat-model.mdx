...
- **Minimal hardcoded allowlist.** Only the domains required for Claude Code itself to function (API, OAuth, telemetry) and Docker image pulls are allowed by default. See the [Security](/security#hardcoded-allowed-domains) page for the canonical list. GitHub, npm, PyPI, and every other service must be explicitly added.
