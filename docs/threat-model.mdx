...
- **TCP/SSH port-level routing.** Non-TLS protocols (SSH, raw TCP) cannot be inspected at the domain level — there is no SNI equivalent for raw TCP connections. Instead, iptables redirects all traffic on the configured port to Envoy, which forwards it to the whitelisted domain's resolved IP. This means a `proto: ssh` rule for `github.com` on port 22 captures **all** outbound port 22 traffic from the container, regardless of the intended destination IP, and routes it to GitHub. An attacker cannot reach an arbitrary SSH server — every port 22 connection ends up at GitHub. The tradeoff is that two different domains cannot be whitelisted on the same non-TLS port. DNS resolution for the upstream target is handled by Envoy at connection time. **If you are pentesting clawker's firewall:** be aware that a successful TCP connect on a port with a whitelisted rule (e.g., port 22) does **not** mean you have reached your intended target. The connection was silently redirected to the whitelisted service (e.g., GitHub, GitLab). Any data you send is going to that corporate service, not your C2. Verify by checking the remote banner or certificate before concluding you have egress.
