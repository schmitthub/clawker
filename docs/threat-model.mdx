...
### A note on supply chain attacks
...
<Danger>
**Running containers with `--privileged` grants elevated permissions and can expose the host system to security risks.** Privileged mode disables all security boundaries — seccomp filtering, capability restrictions, device access controls, and mount namespace isolation are all removed. The container effectively has root-level access to the host kernel. Use this option only when absolutely necessary and in trusted environments.

Clawker's security controls (unprivileged user, nosuid mounts, seccomp, capability dropping) are meaningless in privileged mode. If you pass `--privileged`, you are accepting full responsibility for what the agent does.
</Danger>
