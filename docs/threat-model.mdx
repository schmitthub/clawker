...
Regardless of how or if your container became infected, the exfiltration would have failed and cleaning the infection would involve simply building a new image with updated packages, and creating a fresh container.  
...
This layered approach means traffic is controlled at the DNS level (unlisted domains never resolve), the TCP level (Envoy enforces SNI matching, protocol rules, and path-level restrictions), and the UDP level (non-DNS UDP is blocked entirely).
