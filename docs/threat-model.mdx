...
The clawker project leverages version pinning and integrity checks across CI, its infra containers, and the embedded Dockerfile template — with one deliberate, documented exception for the Node.js runtime baked into your agent images (see the note below). SCA and SAST scanning checks are used in CI.

<Note>
**Why the baked-in Node.js runtime is pinned to an LTS *line*, not an exact version.** Everywhere else clawker pins exact versions, but the Node.js runtime installed into the images clawker generates for you is pinned to the current **Long-Term Support line** (e.g. Node 24) rather than a single patch release. Each time you build an image, clawker resolves and installs the latest patch within that line.
...
- **Minimal hardcoded allowlist.** Only the domains required for Claude Code itself to function (API, OAuth, telemetry) are allowed by default. See the [Security](/security#hardcoded-allowed-domains) page for the canonical list. GitHub, npm, PyPI, and every other service must be explicitly added. 
<Warning>
...
- **Unprivileged user.** The agent runs as a non-root user (`claude:claude`) inside the container. It cannot escalate privileges, modify system files within the container, or bypass file permission boundaries on mounted directories.
