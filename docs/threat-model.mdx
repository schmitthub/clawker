...
Bypassed traffic is **not** a forensic blind spot. The eBPF cgroup programs emit a `verdict=bypassed` record for every connect/sendmsg/sock_create call that takes the bypass path, with the same attribution and destination 4-tuple as `allowed`/`denied` records. Records land in the `clawker-netlogger` OpenSearch index alongside the other egress streams — see [Egress Observability](/observability) for the record shape and how to query bypass windows after the fact.
