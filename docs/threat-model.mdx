...
- **Immutable CI inputs.** All GitHub Actions and tool binaries are pinned to exact versions or SHA commit hashes, not mutable version tags. Go dependencies are integrity-checked via `go.sum` checksums. The inline Dockerfiles used to build the embedded firewall infrastructure images (eBPF manager and custom CoreDNS) are pinned to a SHA256 digest of `alpine:3.21`, so an attacker cannot substitute a tampered base image by re-tagging upstream.
- **Deterministic build inputs.** Binaries are compiled with `CGO_ENABLED=0` (pure Go, no C toolchain), stripped (`-s -w`), and built with `-trimpath` via GoReleaser. Build metadata (version, commit date) is injected via ldflags at compile time from the tagged commit, not from wall-clock or host environment. Byte-identical reproduction across hosts is not a claimed property — the three Linux embed binaries (`clawker-cp`, `ebpf-manager`, `coredns-clawker`) are produced inside `Dockerfile.controlplane` and are subject to BuildKit non-determinism; the integrity story rests on signed attestations against the CI build, not local re-runs.
...
- **SBOM generation.** A Software Bill of Materials is generated for every release archive using [Syft](https://github.com/anchore/syft) (SPDX-JSON format) and published as a release asset *and* as a separate signed attestation (see below).
- **GitHub artifact attestation.** Two attestation types are produced via `actions/attest@v4`, both signed by the same Sigstore keyless identity (reusable workflow path + tag ref):
    - **One SLSA v1 build-provenance attestation** (`predicateType: https://slsa.dev/provenance/v1`) covering **16 subjects per release**: 4 release archives, 4 unpacked `clawker` CLI binaries (one per `OS_ARCH`), and 8 embedded Linux binaries (`clawkerd`, `clawker-cp`, `ebpf-manager`, `coredns-clawker` × `amd64`, `arm64`). The predicate's `resolvedDependencies` enumerates every input that affected output bytes — source commit, Dockerfile base images by `sha256` digest, the full apt closure inside the BPF-builder stage, Go toolchain version, bpf2go pin, every BPF C source by content hash, every GitHub Action by commit SHA, and content hashes of the workflow + Makefile + goreleaser config. An investigator reading the attestation alone can enumerate every input — completeness of enumeration is the property, not byte-identical reproducibility.
    - **Four SPDX SBOM attestations** (`predicateType: https://spdx.dev/Document`), one per release archive, sourced from the same syft output published as release assets.
  Same-repo SLSA Build L3 isolation is provided by GitHub's reusable-workflow boundary: signing happens in the reusable workflow's isolated job context, so the Fulcio cert SAN cryptographically binds to `release-build.yml@refs/tags/<tag>` and cannot be impersonated by any other workflow in this repo. Browse attestations at [github.com/schmitthub/clawker/attestations](https://github.com/schmitthub/clawker/attestations).
...
**3. Verify SLSA build provenance + SPDX SBOM attestations.** GitHub's verifier auto-fetches the signed bundle from the attestation store. Requires `gh` >= 2.49.
...
`gh attestation verify` returns no stdout on success — only an exit code. Add `--format json | jq` to surface the subject digest, builder ID, and signing identity. Both the SLSA build-provenance attestation and the per-archive SPDX SBOM attestation are signed by the same identity, so a single `gh attestation verify` call resolves all attestations for the archive's digest.

### Per-binary forensic verification

Because the SLSA build provenance covers **16 subjects** — not just the 4 archives — an investigator can verify any individual binary that came out of a clawker release without re-downloading the archive. This matters when:
...
```bash
# Unpack the archive (or pull the binary out of a running container).
tar -xzf /tmp/verify/clawker_0.7.6_linux_arm64.tar.gz -C /tmp/verify

# Verify the unpacked CLI binary against the attestation.
# Note: the bundle is the same one signed at build time; gh resolves it by digest.
gh attestation verify /tmp/verify/clawker \
  --owner schmitthub \
  --signer-workflow schmitthub/clawker/.github/workflows/release-build.yml
```

The same call works against any of the embedded Linux binaries (`clawkerd`, `clawker-cp`, `ebpf-manager`, `coredns-clawker`) extracted from a running CP container or a forensic disk image. The signing identity is the reusable workflow at the release tag; the subject set inside the signed predicate must contain that binary's `sha256` digest, or verification fails.

The `resolvedDependencies` block of the predicate is what makes "what built this" auditable from the attestation alone. To dump it:

```bash
gh attestation verify /tmp/verify/clawker --owner schmitthub \
  --signer-workflow schmitthub/clawker/.github/workflows/release-build.yml \
  --format json | jq '.[].verificationResult.statement.predicate.buildDefinition.resolvedDependencies'
```

Entries are categorized by `name` prefix (`base-image-*`, `apt-*`, `action-*`, `bpf-source-*`, `tool-*`, `config-*`) so you can filter by input class.
...
All clawker containers run on a dedicated `clawker-net` bridge network, isolated from other Docker workloads and the host's network. Only internal traffic is freely allowed, but Docker's built-in DNS service discovery only resolves clawker's own internal services (Envoy, CoreDNS, the host proxy via `host.docker.internal`, and the optional monitoring stack). The network gateway — which routes to the host machine — is locked down separately: only the host proxy port is allowed through, and all other traffic to the gateway is redirected to Envoy and denied. This prevents the agent from reaching host services even though it has network-level access to the bridge subnet.

Claude Code runs as an unprivileged `claude` user (UID 1001) inside the container. Routing is managed by the eBPF manager, which attaches cgroup programs per protocol from outside the container — no container capabilities are needed.
...
All egress traffic going through the firewall stack is visible in the Grafana dashboard when the monitoring stack is running, including domain, protocol, HTTP path, response codes, and DNS query details. (see: [Monitoring](/monitoring))
