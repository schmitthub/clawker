...
Envoy and CoreDNS emit structured access logs (domain, protocol, HTTP path, response codes, DNS query details) to stdout — inspect them via `docker logs clawker-envoy` and `docker logs clawker-coredns`. Routing these into OpenSearch via the OTel Collector is on the roadmap. (see: [Monitoring](/monitoring))
