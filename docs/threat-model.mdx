...
<Note>
Claude Code's `WebFetch` and `WebSearch` tools use an Anthropic-managed proxy to access the internet. These requests do not go through the clawker firewall — they are routed through Anthropic's infrastructure, which curates and controls the access. This means the agent may appear to reach domains that are not in your allowlist, but this is expected behavior, not a firewall bypass. The proxy is managed by Anthropic, not clawker.
</Note>
...
Use small time windows (e.g., `clawker firewall bypass 30s --agent dev`). Pay attention while it's active. If you notice anything suspicious, immediately close the bypass with `clawker firewall bypass --stop --agent <agent-name>`. During a bypass, all outbound traffic goes directly to the network without filtering — the firewall is fully off for that container. Don't leave long bypasses running unattended.
