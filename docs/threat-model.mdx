...
The SLSA build-provenance attestation covers **twenty subjects** — the four release archives, the four `clawker` CLI binaries, and the twelve embedded Linux binaries — so an investigator can verify any individual binary without re-downloading the archive. This matters when:
...
- You have only one of the twelve embedded Linux binaries from a forensic image and need to answer "which release did this come from, and is it intact?"
...
Every reproducibility input lives in the source tree at the attested commit — `Makefile` for the BPF apt pins, `controlplane/firewall/ebpf/gen.go` for the bpf2go pin and clang flags, the workflow YAMLs for action SHAs. Resolve them via `git show <commit>:<path>` once you have the source commit from the attestation.
...
- **CoreDNS (`clawker-coredns`)** runs with `CAP_BPF + CAP_SYS_ADMIN` and a `/sys/fs/bpf` bind mount — the minimum required to write resolved `IP → route identity` entries into the pinned `dns_cache` map at every successful DNS reply. See [Firewall → Architecture](/firewall#architecture).
...
- **Raw TCP port-level routing.** TLS and HTTP carry domain metadata (SNI, Host header) that Envoy can inspect, allowing multiple domains per port. Raw TCP protocols like SSH have no such field — there's nothing in the stream identifying the intended domain. IP-based rules are not viable because IPs rotate frequently behind CDNs and load balancers. Instead, each `ssh`/`tcp` rule gets its own dedicated Envoy listener keyed by `(domain, port)`, redirected to by a dedicated eBPF route. A `proto: ssh` rule for `github.com` on port 22 routes only GitHub's resolved traffic there; a second rule for `gitlab.com` on the same port gets its own independent listener — multiple domains can share a destination port without collision, and an attacker cannot reach an arbitrary SSH server. An unresolved IP that never went through CoreDNS has no matching route and is denied. **If you are pentesting clawker's firewall:** a successful TCP connect on a port with a whitelisted rule does **not** mean you reached your intended target. The connection was silently redirected to the whitelisted service. Verify by checking the remote banner or certificate before concluding you have egress.
...
- **Unprivileged user.** The agent runs as the unprivileged container user (`clawker`) inside the container. It cannot escalate privileges, modify system files within the container, or bypass file permission boundaries on mounted directories.
...
<Warning>
**Current gap:** Bind mode and worktree containers leave the shared repository's `.git/hooks` writable by the agent. A hook planted there will run the next time you invoke git on the host in that directory. Until the planned policy controls below ship, prefer **snapshot** mode for unattended or high-risk sessions, and review `.git/hooks` before running git on the host against a repo an agent has touched.
</Warning>
...
### Third-Party Bundle Trust

Installed bundles supply stacks, harnesses, and monitoring extensions — components that introduce direct system behaviors. Stack and harness fragments execute during image build (including as root), a harness defines the agent's entrypoint and its default egress floor (the domains every container built from it may reach), and monitoring extensions ship dashboards and pipelines into the observability stack. Bundle validation checks structure, not intent — it will not catch a malicious component.

- Only install bundles from sources you trust, and review their contents before adding them to `clawker.yaml`.
- Consider forking a third-party bundle repository and installing from your fork, so updates only reach you after you've reviewed them.
- Stay diligent when updating bundles — `bundle update` pulls whatever the source now publishes.
- Treat a bundle's declared egress floor as part of your firewall review: a harness you install decides which domains its containers can reach by default.
