...
The note is suppressed when you're not at an interactive terminal (output
piped or redirected, `CI` set) and can be opted out entirely by setting
`CLAWKER_NO_UPDATE_NOTIFIER` — the same switch that silences the new-version
update notifier.
