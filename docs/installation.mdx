...
Downloads a pre-built binary from GitHub releases. No Go toolchain required. The install script itself is published as a release asset, so the URL is pinned to a tagged release rather than a moving branch.
...
Requires Go 1.25+. Use the Makefile entry point — `go install` is unsupported because the embedded Linux binaries are gitignored and produced by `make release-embeds`.
