...
| Field | Purpose |
|-------|---------|
| `source` | Host socket path. Supports a leading `~`, `$VAR`, and `${VAR}`. |
...
The `source`, `target`, and `purpose` fields are required. Clawker resolves the
source path and asks the user to approve the exact socket and its listener
identity. See [Host socket bridges](/host-sockets) for the user workflow.
