...
**`clawker run` / `clawker container start` fails with `control plane clock did not catch up to host` (Docker Desktop).**
...
```text
control plane clock did not catch up to host after 30s: CP clock still behind host (CP at 2026-06-08T12:00:00Z) — the Docker VM clock is likely lagging after host sleep; wait for it to re-sync (or restart Docker Desktop), then retry
```
