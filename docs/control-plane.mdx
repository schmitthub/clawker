...
- **Agent watcher + clean self-shutdown** — polls Docker every 30s for `purpose=agent, managed=true` containers. After drain-to-zero (`missed_threshold × pollInterval + 60s` grace), it fires an ordered drain callback: cancel bypass timers → graceful gRPC stop → Stack stop → `netlogger.Stop` (drains the eBPF egress event pipeline and flushes the OTLP BatchProcessor BEFORE BPF maps go away) → eBPF `FlushAll` → exit code 0. The `on-failure` restart policy does not retrigger.
...
**`clawker run` / `clawker container create` fails with `control plane clock not in sync with host` (Docker Desktop).**
Before creating a container, the CLI brings the control plane to full readiness — which includes checking that the host clock and CP's clock (the Docker Desktop LinuxKit VM clock, where CP runs) agree within a couple of seconds. If they don't converge within ~30s the command fails with:

```text
control plane clock not in sync with host after 30s: last measured offset 7.4s exceeds tolerance 2s — the Docker VM clock is likely lagging after host sleep; wait for it to re-sync (or restart Docker Desktop), then retry
```

This almost always happens after the host sleeps and wakes: the VM clock drifts behind real time until its NTP source re-syncs. The check exists because CP's auth (Hydra) validates the CLI-signed bootstrap assertion against CP's clock with zero leeway — creating a container against a drifted clock would otherwise bake unusable bootstrap material into it, with no way to re-mint (you'd have to delete and recreate). Wait a few seconds for the VM clock to catch up and retry, or restart Docker Desktop to force a resync. (The earlier symptom of this drift — a Hydra `Token used before issued` 500 at container start — is now caught by this pre-create check instead.)
