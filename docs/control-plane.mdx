...
On every boot, CP reads `firewall.enable` from settings and — when enabled (the default) — starts the Envoy + CoreDNS firewall stack itself in the background. That covers boots no CLI command observes, like Docker's restart policy resurrecting a crashed CP. A failed stack bringup degrades (CP stays up and logs it) rather than crashing; the next `clawker firewall up` or container start retries it.
