...
clawker's control plane runs **netlogger** — a userspace pipeline that drains an eBPF ringbuf populated by the cgroup/connect/sendmsg/sock_create programs and emits one OTLP log record per egress decision. netlogger starts as part of CP's own boot sequence, independent of whether the firewall subsystem (Envoy + CoreDNS) is enabled — it degrades to a no-op when its OTLP export chain is unavailable, most commonly when no OTLP collector endpoint is configured. Every connect, sendmsg, and socket-create call from a managed agent container produces a record carrying the kernel's verdict (`allowed` / `denied` / `bypassed`), the container's attribution (`agent`, `project`, `container_id`), the destination 4-tuple, and the resolved domain when DNS context is available.
...
| `bpf_ts_ns` | keyword | Kernel monotonic timestamp at the moment of decision (`bpf_ktime_get_ns`) — stored as an opaque string, never numeric math |
...
| `identity` | keyword | Route identity allocated by the control plane per firewall destination. Correlates userspace records with BPF `dns_cache` / `route_map` entries when `dst_host` is empty (direct-IP connect, rule removed mid-flight, stale dnsbpf entry) |
...
agent container egress decision
  ...
  OTLP/gRPC + mTLS via netlogger's *sdklog.LoggerProvider
   (built by controlplane/otel.NewOtelLoggerProvider —
...
Identities are allocated — not derived — so two destinations can never share one (Cilium's identity-allocation pattern). An identity stays bound to its destination for as long as the destination remains configured, surviving rule churn and control-plane restarts.
