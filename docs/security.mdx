...
Clawker is a self-hosted AI coding agent sandbox: every agent runs in an isolated Docker container, unprivileged and behind a deny-by-default egress firewall. Network isolation is the foundation — prompt injection can change an agent's intent, but it cannot change the container's constraints: a coerced agent can't touch your host system, and it can't send data to any destination outside your allowlist, so exfiltration to an arbitrary attacker is denied at the network boundary. This is how you run coding agents like Claude Code and Codex safely, including with permission prompts disabled, on your own machine — free, local, no cloud. This guide explains the container security controls and how to customize them.
...
This is intentional. The default allowlist is the selected harness's **egress floor** — each harness bundle ships the bare minimum set of domains that harness needs to function, and it is composed with your project rules. Everything else — including GitHub, npm, PyPI, or any other service your project needs — must be explicitly configured by you.
...
### Harness Egress Floors

The only domains allowed by default come from the selected harness bundle's egress floor — the set that harness needs for API access, authentication, and telemetry.
...
| `registry.npmjs.org` | npm package installs (Claude Code is npm-distributed and its hooks install global packages) |
...
| `auth.openai.com` | Login and token exchange |
| `chatgpt.com` | ChatGPT-account backend — path-scoped to the codex backend prefixes (`/backend-api/codex/`, `/backend-api/wham/usage`, `/backend-api/ps/`, `/backend-api/plugins/`, `/backend-api/connectors/directory/`); every other path on the host is denied because `chatgpt.com` also hosts publicly readable shared conversations, an exfiltration sink. A companion `wss` rule enables WebSocket upgrades on the same allowed paths for streaming responses |

Custom harness bundles declare their own floor in the bundle manifest's `egress:` list.
...
Images built for the claude harness include an agent prompt file at `/etc/claude-code/CLAUDE.md`. This file is automatically loaded by Claude Code and gives the agent awareness of its containerized environment:
...
The container's PID 1 is `clawkerd`, the per-container daemon that supervises the harness. clawkerd runs as root inside the container, but it never executes user-controlled code in its own process. The user CMD (the harness CLI) is forked with kernel-side privilege drop via `SysProcAttr.Credential` — between `fork()` and `execve()` the kernel performs `setgroups → setgid → setuid` atomically, so there is no user-mode code path between root and the unprivileged container user (`claude`, UID baked at build time — host-derived on Linux for bind-mount writability, 1001 elsewhere).
...
Clawker ships a **custom CoreDNS build** with an in-tree `dnsbpf` plugin (`internal/dnsbpf/`). The plugin intercepts every DNS response for an allowlisted zone, extracts the A records, and writes each `(ip → domain_hash, ttl)` entry to the pinned `dns_cache` map in real time. Live per-query tracking is immune to DNS round-robin rotation: startup-time seeding would go stale as CDNs cycled backends, silently blackholing traffic until a firewall reload. The plugin-based approach tracks whatever the upstream resolver returns on every query, so any IP a container actually uses has been observed and hashed at the moment of connection.
