...
The firewall no longer requires any container capabilities. eBPF cgroup programs are attached from outside the container by the eBPF manager (which runs with `CAP_BPF`), so agent containers need no special privileges for traffic routing. Raw socket creation is blocked by the eBPF `sock_create` program rather than by container capabilities.

The `cap_add` field is available if your workflow needs additional Linux capabilities:
