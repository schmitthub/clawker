...
ownership — so pick a distinctive one. The `clawker` namespace and
impersonation forms of it are reserved and rejected.
...
An identity is claimed by exactly one source at a time. If a bundle with the
same `namespace` and `name` is already installed in the cache — say you
previously installed your own published release — clawker refuses to resolve
until you purge it (`clawker bundle remove <namespace>.<name>`) or drop one of
the declarations. A local directory never silently stands in for an installed
bundle.
