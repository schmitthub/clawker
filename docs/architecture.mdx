...
│              │  │                     │  │                            │
│ docker/      │  │ storage/ (engine)   │  │ firewall/ (Envoy+CoreDNS+  │
│ workspace/   │  │ config/ (project)   │  │            eBPF manager)   │
│ containerfs/ │  │ config/ (settings)  │  │ ebpf/  (BPF loader/maps)   │
│ bundler/     │  │ project/ (registry) │  │ dnsbpf/ (CoreDNS plugin)   │
│              │  │ storeui/ (TUI edit) │  │ hostproxy/ (auth)          │
│ pkg/whail    │  │                     │  │ socketbridge/ (SSH)        │
│ (engine lib) │  │                     │  │ keyring/ (creds)           │
└──────┬───────┘  └─────────────────────┘  └────────────────────────────┘
       │
       ▼
...
| `firewall` | config, logger, storage | Envoy + CoreDNS + eBPF firewall: daemon, config gen, PKI, rules, embedded image builds |
| `dnsbpf` | ebpf | CoreDNS plugin that writes DNS resolutions to the BPF `dns_cache` map in real time (embedded in `cmd/coredns-clawker`) |
...
| `firewall.FirewallManager` | `internal/firewall` | Envoy + CoreDNS + eBPF firewall lifecycle, rules, bypass |
| `dnsbpf.Handler` | `internal/dnsbpf` | CoreDNS plugin populating the BPF `dns_cache` map in real time |
| `ebpf.Manager` | `internal/ebpf` | BPF program loader; `SyncRoutes` replaces the global `route_map` atomically |
| `cmd/coredns-clawker` | `cmd/coredns-clawker` | Custom CoreDNS entrypoint embedding `dnsbpf`; built into `clawker-coredns:latest` by the firewall manager on demand |
