...
keywords: ["AI coding agent sandbox", "coding agent sandbox", "run coding agents in Docker", "Claude Code", "Codex", "self-hosted", "open source", "architecture", "Go CLI"]
---
...
│  17 command groups, 80+ subcommands (Cobra, Factory DI)                    │
└──────┬──────────────────────────┬──────────────────────┬────────────────────┘
       │                          │                      │
       ▼                          ▼                      ▼
┌──────────────┐    ┌─────────────────────┐    ┌────────────────────────────┐
...
└──────┬───────┘    └─────────────────────┘    │ Ory auth stack (Hydra,     │
  ...
  moby/moby (Docker SDK)                       │ Pub/sub event bus          │
                                               │ Owns: firewall (Envoy +    │
                                               │  CoreDNS) + eBPF cgroup    │
...
| `firewall` | status, list, add, remove, reload, refresh, prune, up, down, enable, disable, bypass, rotate-ca | Egress firewall control (all RPCs route through `f.AdminClient` to the CP daemon) |
...
| `monitor` | init, up, reload, down, status, extensions | Observability stack (OTel Collector, OpenSearch, OpenSearch Dashboards, Prometheus) |
| `settings` | edit | User settings TUI editor |
| `plugin` (alias `skill`) | install, show, remove | Manage the clawker-support agent skills plugin |
| `alias` | set, list, delete, export | Manage command aliases |
| `bundle` | install, list, update, validate, remove, prune | Manage distributed bundles of harnesses, stacks, and monitoring extensions |
| `harness` | list | Inspect resolvable harnesses |
| `stack` | list | Inspect resolvable stacks |
| `prompt` | print | Access clawker's managed agent briefing |

**Top-level shortcuts**: `init` → `project init`, `build` → `image build`, `run`/`start` → `container run`/`start`, `version`
...
| `controlplane/agent` | auth, consts, dockerevents, pubsub, logger | CP-side agent surface: Dialer, sqlite Registry, Register handler, IdentityInterceptor, session/agent events |
| `controlplane/pubsub` | logger | Generic typed pub/sub engine (`Topic[T]`/`Event[T]`); each domain (e.g. `controlplane/agent`) projects its own worldview from subscribed events — there is no shared central state store |
| `controlplane/dockerevents` | pubsub, logger | Docker events feeder (reconnecting stream → typed events on a pub/sub topic) |
| `controlplane/manager` | config, docker, logger (+ embedded clawkercp + ebpf-manager + bpffs-delegate binaries) | Host-side CP lifecycle: `Manager.Start`/`Stop`/`CPRunning` |
...
| `controlplane/firewall/ebpf/netlogger` | controlplane/firewall/ebpf, controlplane/pubsub, controlplane/dockerevents, controlplane/otel, config, logger | Per-decision-point egress event emitter — drains the BPF `events_ringbuf`, enriches by cgroup ID with container/agent/project attribution via the eBPF-enrolled pub/sub topic, emits OTLP log records via a `*sdklog.LoggerProvider` (`service.name=ebpf-egress`) |
...
| `adminclient.Dial` | `controlplane/adminclient` | CLI-side AdminService gRPC dial (mTLS + OAuth2) |
| `firewall.Handler` | `controlplane/firewall` | 13-method AdminService firewall surface inside the CP daemon |
| `firewall.Stack` | `controlplane/firewall` | Envoy + CoreDNS container lifecycle |
| `dnsbpf.Handler` | `internal/dnsbpf` | CoreDNS plugin populating the BPF `dns_cache` map in real time |
| `ebpf.Manager` | `controlplane/firewall/ebpf` | BPF program loader; `SyncRoutes` replaces the global `route_map` atomically |
| `netlogger.Service` | `controlplane/firewall/ebpf/netlogger` | Drains BPF `events_ringbuf`, enriches by `cgroup_id`, emits OTLP log records (`service.name=ebpf-egress`) on the trusted infra lane |
| `otel.NewOtelLoggerProvider` | `controlplane/otel` | Generic `*sdklog.LoggerProvider` factory (mTLS + infra cert, BatchProcessor, optional ExporterWrap) reused by netlogger and any future CP-side OTLP log emitter |
| `manager.Manager` | `controlplane/manager` | Host-side CP container lifecycle (`Manager.Start`/`Stop`/`CPRunning`) |
| `agent.Registry` | `controlplane/agent` | sqlite-backed agent identity (CP is sole writer) |
| `pubsub.Topic[T]` | `controlplane/pubsub` | Generic typed pub/sub engine; each domain (e.g. `controlplane/agent`) projects its own worldview from subscribed events |
