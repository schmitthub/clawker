...
security:
  firewall:
    # Per-project firewall rules (global enable/disable is in settings.yaml)
    # Add domains to the default allowed list
    # add_domains:
    #   - "api.openai.com"
    # IP range sources (DEPRECATED — ignored at runtime, has no effect)
    # ip_range_sources:
    #   - name: github
    # Full rule specification (for path-level filtering)
    # rules:
    #   - dst: "api.example.com"
    #     proto: tls
    #     port: 443
...
The `context` field sets a custom build context directory. Build-time arguments can be passed via the `--build-arg` flag on `clawker image build`.
...
`editor`/`visual` override the default editor inside the container. Use `--memory` on `clawker run` to set a container memory limit.
...
The `cap_add` field adds Linux capabilities to the container. The defaults (`NET_ADMIN`, `NET_RAW`) are required for iptables DNAT rules that redirect traffic to the firewall proxy. Only modify this if you know what you're doing.

<Note>
See the [Firewall](/firewall) guide for details on how rules work, the Envoy+CoreDNS architecture, and CLI management commands.
</Note>
...
```yaml
# Global firewall toggle (per-project rules are in .clawker.yaml)
firewall:
  enable: true
