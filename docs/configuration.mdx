...
agent:
  ...
  claude_code:
    ...
    # Bind mount the harness's host state dirs (e.g. ~/.claude/projects/) into the container so auto-memory and sessions are shared across container runs and instances
...
| `mount_projects` | boolean | `true` | Bind mount the harness's host state dirs (e.g. ~/.claude/projects/) into the container so auto-memory and sessions are shared across container runs and instances |
