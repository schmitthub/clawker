...
# Command aliases shared with the team via 'clawker alias import' and 'clawker alias export'; never applied automatically
aliases:  # default: n/a | required: false
  <key>: <value>
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `aliases` | key-value map | — | Command aliases shared with the team via 'clawker alias import' and 'clawker alias export'; never applied automatically |
...
# Command aliases expanded before execution; the value is appended to 'clawker' and supports $1..$N placeholders; an empty value disables an alias
aliases:  # default: dev=run --rm -it --agent dev @ --dangerously-skip-permissions | required: false
  <key>: <value>
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `aliases` | key-value map | `dev=run --rm -it --agent dev @ --dangerously-skip-permissions` | Command aliases expanded before execution; the value is appended to 'clawker' and supports $1..$N placeholders; an empty value disables an alias |
...
The `aliases` key in `settings.yaml` defines command shortcuts that are expanded before execution. The alias value is appended to `clawker` in place of the alias name, and any further arguments are appended after it:

```yaml
aliases:
  dev: run --rm -it --agent dev @ --dangerously-skip-permissions
  lg: logs $1 --tail $2
```

With this configuration, `clawker dev --model opus` runs `clawker run --rm -it --agent dev @ --dangerously-skip-permissions --model opus`, and `clawker lg web 50` runs `clawker logs web --tail 50`. Values may reference positional arguments as `$1`..`$N`; arguments beyond the highest placeholder are appended.

Clawker ships the `dev` alias above as a default. Manage aliases with the `clawker alias` command group (`set`, `list`, `delete`), by editing `settings.yaml` directly, or interactively via `clawker settings edit`.

Rules and limitations:

- An alias can never shadow a built-in command — the real command always wins.
- Setting an alias's value to an empty string disables it. This is also how `clawker alias delete` turns off a shipped default, since defaults can't be removed outright.
...
- Because the alias command disables flag parsing, even `--help` is forwarded into the expansion.

### Sharing Aliases with Your Team

The project config (`clawker.yaml`) also accepts an `aliases` key, but it is a sharing vehicle only — **project aliases are never applied automatically**. A cloned repository cannot silently define command names on your machine.

```bash
# Adopt the aliases shared in the project config (deliberate, validated)
clawker alias import

# Publish your active aliases into the project's clawker.yaml
clawker alias export
```

Import validates each entry and skips anything that shadows a built-in command; existing aliases are kept unless `--clobber` is given. Export writes to the project's shared config file (never a `.local.` override) so the aliases can be committed and adopted by teammates with `clawker alias import`.
