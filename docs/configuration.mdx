...
monitoring:
  ...
  # Host-loopback port for the mTLS-gated OTLP receiver dedicated to trusted clawker infrastructure (CP, Envoy, CoreDNS). require_client_auth is enforced; peers must present a cert chained to the CLI root CA. Agents on clawker-net have no such cert so the TLS handshake rejects them
...
| `otel_infra_port` | integer | `4319` | Host-loopback port for the mTLS-gated OTLP receiver dedicated to trusted clawker infrastructure (CP, Envoy, CoreDNS). require_client_auth is enforced; peers must present a cert chained to the CLI root CA. Agents on clawker-net have no such cert so the TLS handshake rejects them |
