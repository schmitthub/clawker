  ...
  firewall:
    ...
    rules:
      ...
      - dst: <string>  # default: n/a | required: false
        ...
        # Fine-grained path filtering (works with proto tls and http)
...
monitoring:
  ...
  # Host-loopback port for the mTLS-gated OTLP receiver dedicated to clawker-cp push (gates with the CLI-issued OTEL keypair; agents on clawker-net cannot present a client cert and so the TLS handshake fails)
  otel_cp_port: <integer>  # default: 4319 | required: false
  ...
  # Host port for Prometheus metrics UI (also the in-cluster port agents push OTLP metrics to)
  ...
  telemetry:
    # OTEL collector HTTP path for metrics (unused by Claude Code metrics — see prometheus_otlp_path — but retained for non-agent OTLP metric pushers)
    metrics_path: <string>  # default: /v1/metrics | required: false
    # HTTP path on Prometheus' OTLP receiver — Claude Code metrics push here directly, bypassing the OTEL collector
...
| `otel_cp_port` | integer | `4319` | Host-loopback port for the mTLS-gated OTLP receiver dedicated to clawker-cp push (gates with the CLI-issued OTEL keypair; agents on clawker-net cannot present a client cert and so the TLS handshake fails) |
...
| `prometheus_port` | integer | `9090` | Host port for Prometheus metrics UI (also the in-cluster port agents push OTLP metrics to) |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `metrics_path` | string | `/v1/metrics` | OTEL collector HTTP path for metrics (unused by Claude Code metrics — see prometheus_otlp_path — but retained for non-agent OTLP metric pushers) |
| `prometheus_otlp_path` | string | `/api/v1/otlp/v1/metrics` | HTTP path on Prometheus' OTLP receiver — Claude Code metrics push here directly, bypassing the OTEL collector |
...
| State | `~/.local/state/clawker/` | Log files, PID files, update cache |
