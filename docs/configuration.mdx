...
logging:
  ...
  otel:
    # Send logs to the OTEL collector for OpenSearch visibility
...
monitoring:
  ...
  # Host-loopback port for the mTLS-gated OTLP receiver dedicated to trusted clawker infrastructure (CP, Envoy, CoreDNS). Setting client_ca_file on the receiver forces tls.RequireAndVerifyClientCert; peers must present a cert chained to the CLI root CA. Agents on clawker-net have no such cert so the TLS handshake rejects them
  ...
  # Host port the otel-collector exposes its Prometheus scrape endpoint on (Prometheus scrapes this for collector + agent metrics; not Prometheus' own self-metrics)
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | boolean | `true` | Send logs to the OTEL collector for OpenSearch visibility |
...
| `otel_infra_port` | integer | `4319` | Host-loopback port for the mTLS-gated OTLP receiver dedicated to trusted clawker infrastructure (CP, Envoy, CoreDNS). Setting client_ca_file on the receiver forces tls.RequireAndVerifyClientCert; peers must present a cert chained to the CLI root CA. Agents on clawker-net have no such cert so the TLS handshake rejects them |
...
| `prometheus_metrics_port` | integer | `8889` | Host port the otel-collector exposes its Prometheus scrape endpoint on (Prometheus scrapes this for collector + agent metrics; not Prometheus' own self-metrics) |
