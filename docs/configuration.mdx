...
security:
  firewall:
    # Shorthand: domains the container can reach over HTTPS (converted to http+port-443 rules)
    ...
    rules:
      ...
      - dst: <string>  # default: n/a | required: false
        # L7 protocol: http (TLS-terminated, MITM-inspected; default) or any opaque L7 name (e.g. ssh, redis) for TCP pass-through
        proto: <string>  # default: n/a | required: false
        # Override the default port (443 for http)
        ...
        # Fine-grained path filtering (only applies when proto is http)
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `add_domains` | string list | — | Shorthand: domains the container can reach over HTTPS (converted to http+port-443 rules) |
