...
At each directory level during walk-up, each config file resolves independently through three placement styles, in precedence order:

| Style | Path |
|-------|------|
| **Directory form** | `.clawker/clawker.yaml` |
| **Dotted directory form** | `.clawker/.clawker.yaml` |
| **Flat dotfile form** | `.clawker.yaml` |

The first style found wins for that file. Because each file resolves independently, placements can be mixed at the same level — for example a committed flat `.clawker.yaml` beside a `.clawker/clawker.local.yaml` override. A flat file is only shadowed by a directory-form file of the same name. Both `.yaml` and `.yml` extensions are accepted.
...
```
1. .clawker.local.yaml in CWD (any placement style)
2. .clawker.yaml in CWD (any placement style)
3. .clawker.local.yaml in parent directory
4. .clawker.yaml in parent directory
5. ... (continues up to project root)
6. .clawker.local.yaml at project root
7. .clawker.yaml at project root
