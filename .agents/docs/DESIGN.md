...
Clawker is a Go CLI tool that runs coding agents in secure, reproducible Docker containers.
...
See also `.agents/rules/dependency-placement.md` (read when its file patterns match).
...
**Path rules and MITM inspection**: Envoy terminates TLS for every allowed HTTPS/WSS rule and checks the upstream server certificate. Path rules restrict the decrypted HTTP request. For wildcard rules, the CP SDS service supplies a leaf certificate with the exact requested SNI as its SAN. This permits names with multiple subdomain labels. Exact-domain, IP, CIDR, and QUIC chains keep static certificates. QUIC clients need TCP fallback for names that exceed the static wildcard certificate's label depth.
...
See `internal/config/AGENTS.md` for detailed test helper documentation.
