---
name: firewall-uat
description: Live egress probes that prove firewall enforcement from inside a clawker container. Use after any change to controlplane/firewall generator or rule code, before you declare that work complete, and for an authorized firewall UAT task. Golden files and envoy validate do not cover this.
---
...
CP↔generator **contract** gaps. A past regression dropped the Envoy health
listener: golden files and validate passed, and CP bringup hung on the readiness
probe. Run live UAT before declaring the generator branch done.
