...
| `cmdutil.PrintNextSteps` | 23 | 12 | Inline `fmt.Fprintf(ios.Out, ...)` numbered steps |
| `cmdutil.PrintStatus` | 0 | 0 | `if !quiet { fmt.Fprintf(ios.Out, format+"\n", args...) }` |
| `cmdutil.OutputJSON` | 0 | 0 | Inline `json.NewEncoder(ios.Out)` with `SetIndent` |
| `cmdutil.PrintHelpHint` | 0 | 0 | `fmt.Fprintf(ios.Out, "\nRun '%s --help'...\n", cmdPath)` |
...
fmt.Fprintf(ios.Out, "  1. Run 'clawker run %s' to start the container\n", agent)
fmt.Fprintln(ios.Out, "  2. Run 'clawker config check' to verify your configuration")
```
...
// AFTER
if !opts.Quiet {
    fmt.Fprintf(ios.Out, "%s Container %s started\n", cs.SuccessIcon(), name)
}
```
...
// Check stdout for static status messages.
assert.Contains(t, out.String(), "Container started")

// Warnings and errors are absent in this successful case.
