#!/usr/bin/env bash
#
# check-agents-freshness.sh — Compare AGENTS.md/rules timestamps against Go source files.
...
usage() {
    cat <<'EOF'
Usage: check-agents-freshness.sh [OPTIONS]

Compare AGENTS.md and rules file timestamps against Go source files.
...
if [[ "$NON_INTERACTIVE" != "true" ]]; then
    echo -e "${BOLD}=== Agent Documentation Freshness Check ===${RESET}"
    echo ""
fi
...
# ── Check Lazy Loaded AGENTS.md ────────────────────────────────────────────────
while IFS= read -r doc; do
    dir=$(dirname "$doc")
    check_doc "$doc" "$dir"
done < <(git ls-files --cached --others --exclude-standard ':(glob)**/AGENTS.md' | sort -u | while IFS= read -r path; do [[ "$path" == AGENTS.md || ! -f "$path" ]] || printf '%s\n' "$path"; done)

# ── Check .agents/rules/*.md with paths: frontmatter ─────────────────────────
while IFS= read -r rule; do
    ...
    # Select tracked Go files with the exact rule patterns, including filename globs.
    go_paths=()
    while IFS= read -r path; do
        [[ "$path" != *.go ]] || go_paths+=("$path")
    done < <(git ls-files -- "${pathspec_args[@]}")
    local_result=""
    if (( ${#go_paths[@]} > 0 )); then
        local_result=$(git log --format="%at" --diff-filter=ACMR --name-only -1 -- "${go_paths[@]}" 2>/dev/null || true)
    fi
...
done < <(git ls-files --cached --others --exclude-standard '.agents/rules/*.md' | sort -u)
