...
<DataDir>/auth/
├── ca/
│   ├── ca.pem              ← CLI generates (public — bind-mounted into CP)
│   └── ca.key              ← CLI generates (0600, NEVER enters any container)
├── cli/
...
├── certs/
│   ├── cli/
│   │   ├── cert.pem        ← CLI mTLS client cert (signed by CA)
│   │   └── key.pem         ← CLI mTLS client key (0600, NEVER enters any container)
│   └── server/
│       ├── cert.pem        ← CP server cert (signed by CLI CA)
│       └── key.pem         ← CP server key (bind-mounted into CP)
```

**Bind mount into CP (read-only):**
- `auth/ca/ca.pem` — for mTLS client cert verification
- `auth/certs/server/` — CP's TLS server identity
- `auth/cli/signing-jwk.json` — for Hydra `private_key_jwt` client registration

**Never enters any container (0600 permissions):**
- `auth/ca/ca.key` — CA private key
- `auth/cli/signing.key` — CLI's ES256 JWT signing private key
- `auth/certs/cli/key.pem` — CLI's mTLS client cert private key

## Invariants

### INV-B1-001: mTLS is the authentication layer on the gRPC admin API
- **Type**: must
- **Category**: security
- **Statement**: Every connection to the CP gRPC admin API (:7443) requires mutual TLS. The CLI presents its client cert (signed by CLI CA). The CP presents a server cert (signed by CLI CA). mTLS proves identity (authentication). The Hydra token endpoint (:4444) uses TLS + `private_key_jwt` for client authentication, not mTLS (Hydra does not natively support requiring client certs on its public API).
- **Violated when**: A gRPC connection to the CP succeeds without a valid client cert
- **Test approach**: integration (connect to :7443 without client cert → rejected; connect with untrusted cert → rejected; connect with CLI-CA-signed cert → accepted)
- **Risk**: critical
...
### INV-B1-004: Oathkeeper gRPC middleware is the sole authorization enforcer
- **Type**: must
- **Category**: security
- **Statement**: All gRPC admin API authorization is enforced by the vendored Oathkeeper gRPC middleware. No hand-rolled authz interceptors. The middleware validates tokens against Hydra and enforces per-method rules via Oathkeeper rule configuration. Fail-closed on unmatched methods.
- **Violated when**: Authorization logic exists outside of the Oathkeeper middleware, or an unmatched gRPC method is allowed through
- **Test approach**: unit (using Oathkeeper middleware mocks: unmatched method → denied; matched method with valid token → allowed; no custom authz interceptor registered on gRPC server)
- **Risk**: critical
...
- **Type**: must
- **Category**: security
- **Statement**: The CLI's CA private key (`ca.key`), ES256 signing private key (`cli/signing.key`), and mTLS client cert private key (`certs/cli/key.pem`) are never bind-mounted, copied, or otherwise placed inside any container. The CP receives only public halves (CA cert, CLI signing JWK, server cert+key). All private keys are created with 0600 permissions.
- **Violated when**: CA key, CLI signing key, or CLI cert key appears in any container mount config, Dockerfile COPY, or env var
- **Test approach**: unit (assert `HostConfig.Mounts` in CP container creation code excludes all private key paths)
- **Risk**: critical
...
### INV-B1-011: Two CAs never cross-reference
- **Type**: must-not
- **Category**: security
- **Statement**: The CLI auth CA (signs mTLS certs for CLI-CP communication, `<DataDir>/auth/ca/`) and the firewall MITM CA (signs per-domain certs for TLS inspection, `<DataDir>/firewall/certs/`) are separate key material, separate directories, separate rotation commands (`clawker auth rotate` vs `clawker firewall rotate-ca`). Compromise of one must not cascade. A third CA for agent certs (CP CA) is a Branch 4 decision — not specced here.
- **Violated when**: Auth CA signs a domain cert, MITM CA signs an identity cert, or either references the other's key material
- **Test approach**: unit (assert `auth/ca/` and `firewall/certs/` paths are distinct; assert key material is different)
- **Risk**: critical
...
### PRH-B1-002: No hand-rolled auth code
- **Statement**: No hand-rolled token issuance, OIDC endpoints, JWT signing/verification for auth purposes, or authorization interceptors. Token issuance is Hydra. Authorization is Oathkeeper (vendored gRPC middleware or binary). The existing `oidc_provider.go`, `authz.go`, and `oidc_clients.go` are replaced, not patched. CLI assertion signing (for `private_key_jwt`) uses `go-jose/v4` which is acceptable — it's a standard JWT library, not hand-rolled auth server code.
- **Detection**: grep for custom JWT signing outside CLI assertion code, `/token` HTTP handlers, custom authz interceptors
- **Consequence**: Hand-rolled auth is a security liability. Battle-tested libraries exist.
