...
- **Type**: must
- **Category**: functional
- **Statement**: `controlplane.EnsureRunning(ctx, docker, cfg, log) error` is safe under concurrent calls and safe when CP is already running. Returns `nil` if CP is running and healthy; non-nil on any step failure. `clawker-net` network is ensured via existing `whail.Engine.EnsureNetwork` (as the `ContainerCreateOptions.EnsureNetwork` side-effect) — not as a separate call site. **Spec reconciliation**: if an existing CP container is found whose bind-mount mode for `cfg.FirewallDataSubdir()` is read-only (B1 legacy) instead of the required RW (INV-B2-011 complement), `EnsureRunning` stops + removes the stale container and recreates with the correct mount mode. Same reconciliation for any other mount-spec divergence that would silently break future CP writes.
- **Boundary**: TB-002
- **Violated when**: Concurrent bootstraps create duplicate CP containers, partial failure leaves orphan resources, a healthy CP is unnecessarily restarted, or a mount-mode-divergent CP container is kept without recreation.
- **Guards against**: race conditions, orphan resources from crashed bootstraps, wasteful restarts, silent B1→B2 upgrade failure on first rule write.
- **Test approach**: unit (concurrent goroutines with fake `*docker.Client` — assert single container create; fake returns existing container with RO mount — assert stop+recreate with RW); integration (real Docker idempotency + upgrade path from a pre-staged RO-mount container).
- **Risk**: high
