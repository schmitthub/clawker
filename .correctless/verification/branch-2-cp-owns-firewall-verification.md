...
| INV-B2-006 | `TestEnsureRunning_HappyPath_CreatesContainer`, `TestEnsureRunning_AlreadyRunning_IsNoOp`, `TestEnsureRunning_ExistingStopped_StartsWithoutRecreate`, `TestEnsureRunning_MountDivergence_RecreatesContainer` (4-case table), `TestEnsureRunning_HealthzTimeout_SurfacesError`, `TestEnsureRunning_ConcurrentCallers_SingleCreate`, `TestEnsureRunning_NameConflictRecovery_NoSecondCreate` | **covered** | Strong unit coverage. Explicit table exercises RO/RW divergence, source-path drift, missing mount, extra mount. Concurrent-goroutine test proves mutex serialization. Real-Docker integration for the RO→RW upgrade path called for by the spec is not present; unit coverage is sufficient for the type of bug this guards against. |
...
- **INV-B2-006**: Real-Docker integration test for the RO→RW mount upgrade path is missing. Unit coverage is strong; integration adds defense-in-depth for the B1→B2 upgrade case.
