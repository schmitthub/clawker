...
import (
	...
	"github.com/claucker/claucker/internal/credentials"
	"github.com/claucker/claucker/internal/dockerfile"
	"github.com/claucker/claucker/internal/engine"
	"github.com/claucker/claucker/internal/term"
	"github.com/claucker/claucker/internal/workspace"
...
func init() {
	...
	upCmd.Flags().StringVarP(&upMode, "mode", "m", "", "Workspace mode: bind or snapshot (default from config)")
	upCmd.Flags().BoolVar(&upBuild, "build", false, "Force rebuild of the container image")
	upCmd.Flags().BoolVar(&upDetach, "detach", false, "Run container in background (detached mode)")
	upCmd.Flags().BoolVar(&upClean, "clean", false, "Remove existing container and volumes before starting")
}
...
func runUp(cmd *cobra.Command, args []string) error {
	...
	if err := ensureImage(ctx, eng, cfg, imageTag); err != nil {
		return err
	}

	// Setup workspace strategy
	wsStrategy, err := setupWorkspace(ctx, eng, cfg, mode)
	if err != nil {
		return err
	}

	// Build container configuration
	containerCfg, err := buildContainerConfig(cfg, imageTag, wsStrategy)
	if err != nil {
		return err
	}
...
func determineMode(cfg *config.Config) (config.Mode, error) {
	if upMode != "" {
		return config.ParseMode(upMode)
	}
	return config.ParseMode(cfg.Workspace.DefaultMode)
}

func ensureImage(ctx context.Context, eng *engine.Engine, cfg *config.Config, imageTag string) error {
...
func setupWorkspace(ctx context.Context, eng *engine.Engine, cfg *config.Config, mode config.Mode) (workspace.Strategy, error) {
...
func buildContainerConfig(cfg *config.Config, imageTag string, wsStrategy workspace.Strategy) (engine.ContainerConfig, error) {
