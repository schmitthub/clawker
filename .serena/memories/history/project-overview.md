...
- **Control Plane (landed 2026-04)**: `clawker-controlplane` container runs `clawkercp` daemon (PID 1) + Ory stack (Hydra/Oathkeeper/Kratos). CLI dials CP via mTLS TCP + OAuth2 JWT (Hydra introspection). CP owns `ebpf.Manager.Load()` in-process. See `internal/controlplane/AGENTS.md`.
