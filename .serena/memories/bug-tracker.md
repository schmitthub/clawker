...
- [ ] **handleAgentReady write/close failure paths uncovered.** `cmd/clawkerd/session.go::handleAgentReady` write-failure (non-`ErrClosedPipe`) and close-failure branches aren't unit-tested. Synthesizing kernel-level write/close errors on a live fifo (EBADF / EFAULT) without dep-injecting the writer/closer would mean either chmod tricks that hit the open path instead, or fragile race timing — both worse than the uncovered branches. Acceptable until either branch becomes a regression vector; revisit if `agentReadyWriter` / `agentReadyCloser` seams are added for other reasons.
