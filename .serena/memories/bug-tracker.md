...
- [ ] **Protocol coverage gaps: UDP, FTP, QUIC, HTTP/3, WebRTC, raw IP** ([#313](https://github.com/schmitthub/clawker/issues/313)). Surveyed during `feat/networking-dashboard` adversarial sweep. Unconnected non-DNS UDP silently dropped at `decide_sendmsg` (no `proto: udp` rule); QUIC/HTTP/3 blocked at UDP layer + no Envoy QUIC listener; FTP-passive unallow-listable (no port-range field on `EgressRule`); `proto: tcp`/`ssh` opaque listeners have no L7 surface (no SNI inspection, no path rules, no `action` metadata). Intra-subnet connected-UDP passthrough is a footgun if any non-infra container ever lands on clawker-net. Issue includes proposed slicing: `proto: udp` first, then port ranges, then SNI-inspecting TCP, then HTTP/3 (biggest lift). Defer until a user actually needs one.
