...
### cred_forwarding
No — (corrected 2026-07-18, attribution audit) none of the documented mechanisms is a mediated forwarding of a developer's own credentials: SSH public-key registration authenticates the developer *to* the instance rather than forwarding credentials *into* it, "agent tokens" scope the sandbox's own access to the Morph control-plane API (unrelated to the developer's git/ssh/gpg credentials), and "Git authentication integration" is undetailed at the source (confirmed via direct fetch of the getting-started, development, and sharing-and-collaboration pages — none describes an ssh-agent/gpg-agent-style proxy). The one concretely documented secrets pattern (e.g. `ANTHROPIC_API_KEY`) is direct configuration inside the remote instance, which the rule explicitly excludes as "creds you pass yourself."
Sources:
