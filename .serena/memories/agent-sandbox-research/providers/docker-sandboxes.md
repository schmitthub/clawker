...
### git_worktrees
git_worktrees: No — (corrected 2026-07-18, attribution rule) the "agents view" worktree fan-out is Claude Code's own subagent feature running inside the sandbox, not a Docker Sandboxes capability; crediting it to the sandbox layer was an attribution error. Docker Sandboxes' own worktree posture is negative and documented: clone mode is explicitly rejected when invoked from a host-side git worktree other than the main checkout (can't resolve the worktree's `.git` pointer through the read-only bind mount) — host-worktree-as-sandbox-input unsupported.
Sources:
