...
- **Viper → storage.Store[T] migration** (2026-02-22): `internal/config` fully migrated from Viper to `storage.Store[T]`. `configImpl` composes `Store[Project]` + `Store[Settings]`. Typed mutation API: `SetProject(fn func(*Project))`, `SetSettings(fn func(*Settings))`, `WriteProject()`, `WriteSettings()`. Registry extracted to `internal/project` as `Store[ProjectRegistry]`. Viper, fsnotify, mapstructure removed from go.mod. Test doubles: `NewBlankConfig()` (defaults), `NewFromString(projectYAML, settingsYAML)` (no defaults), `NewIsolatedTestConfig(t)` (file-backed, writable). `ValidateProjectYAML()` for strict validation.