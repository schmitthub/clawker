...
## Recent Changes (Config Refactor)
- **Config write model + docs/testing guide** (2026-02-18): `internal/config` now uses in-memory dirty tracking (`Set`) with selective persistence on `Write(WriteOptions)`. Project-scope writes resolve to local `./clawker.yaml` when inside a registered project and fall back to user-level config-dir `clawker.yaml` otherwise. Added package-level testing guidance across `internal/config/CLAUDE.md`, `.claude/rules/testing.md`, `.claude/rules/code-style.md`, `.claude/docs/ARCHITECTURE.md`, and `.claude/docs/DESIGN.md` with test-double usage (`NewMockConfig`, `NewFakeConfig`, `ReadFromString`) and focused package test commands.