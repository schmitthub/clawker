...
- **Control Plane (landed 2026-04)**: `clawker-controlplane` container runs `clawker-cp` daemon (PID 1) + Ory stack (Hydra/Oathkeeper/Kratos). CLI dials CP via mTLS TCP + OAuth2 JWT (Hydra introspection). CP owns `ebpf.Manager.Load()` in-process. See `internal/controlplane/CLAUDE.md`.
- **CP ≠ firewall**: CP is unconditional infrastructure (auth, gRPC AdminService + AgentService, agent slot/registry bookkeeping, owns clawker-net). The firewall is one optional subsystem CP manages, toggled by `firewall.enable` in `settings.yaml` (the master switch is global). The project schema's `security.firewall` field holds per-project `add_domains`/`rules` ONLY — not the enable flag. Disabling firewall does NOT disable CP, AnnounceAgent, clawkerd Register, ListAgents, or any non-firewall AdminService RPC. CP owns firewall, not vice versa.
