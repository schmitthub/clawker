...
- **CP ≠ firewall**: CP is unconditional infrastructure (auth, gRPC AdminService + AgentService, agent slot/registry bookkeeping, owns clawker-net). The firewall is one optional subsystem CP manages, toggled by `security.firewall.enable`. Disabling firewall does NOT disable CP, AnnounceAgent, clawkerd Register, ListAgents, or any non-firewall AdminService RPC. CP owns firewall, not vice versa.
