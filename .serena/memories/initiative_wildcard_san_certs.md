...
## Required check correction (2026-09-07)
- The user rejected the earlier hook bypass. All required checks are mandatory. The earlier decision to use `SKIP` was incorrect and is not authorization for any future bypass. Never skip required hooks to get a commit through, including when toolchain or network setup prevents a check from running.
- `.clawker.yaml` already installs Go and the development tools. The current container has Go 1.27.1, golangci-lint 2.13.2 built with Go 1.27.1, and govulncheck 1.7.0. Use those installed tools without adding a separate Go pin or installing another toolchain.
- Local lint reproduced a `goconst` failure for the Envoy bootstrap's cluster key. Added `keyCluster` beside the existing Envoy key constants and used it at all seven occurrences. Existing generated YAML must remain unchanged.
- The SDS test helper's unencrypted bufconn gRPC server produced a Semgrep code-scanning alert despite its suppression comment. Replaced it with a TLS server and a client that verifies the test CA and hostname; removed the suppression and insecure credentials. Existing SDS tests passed before the edit. The focused SDS and Envoy golden tests pass after both changes.
- Local govulncheck 1.7.0 passes with no called vulnerabilities. CI was installing 1.1.4; updated that exact tool version and the hook installation hint to 1.7.0 so CI uses the locally verified scanner. The precise CI scanner error has not been confirmed from its logs.
- Running `prek run --all-files` with the installed tools and no skipped hooks before committing the correction. No `go test ./...` command and no `.golangci.yml` change.

- All individual checks in the first full local hook run passed: tidy, secret scanning, lint, both Semgrep scans, govulncheck, unit tests, generated docs, NOTICE, advisory docs, and plugin drift. The overall runner returned 1 because this ledger was edited during its concurrent scan phase. Source and module diffs contain only the intended fixes. Run the final gate with no concurrent edits, then commit through the same required hooks without bypass.
