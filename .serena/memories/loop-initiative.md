...
**Task 19:**
- Created comprehensive integration tests in `test/agents/loop_test.go` (16 new test functions, ~850 lines) and `test/commands/loop_test.go` (13 new test functions, ~400 lines). Tests are write-only (not runnable in current container environment due to Docker + OOM risk) — to be verified by host-level agent post initiative pre merge.
- **test/agents/loop_test.go** (Runner-level): Tests cover RunSingleIteration, SessionPersistenceAcrossIterations, CircuitBreakerTripsOnStagnation, CircuitBreakerBlocksRerun, ResetCircuitAllowsRerun, CompletionDetection, LOOPSTATUSParsing, NoLOOPSTATUS_CountsAsNoProgress, ContextCancellation, OnOutputCallback, HookInjection (default + custom), SessionExpiration, NamingFormat, RateLimiter state persistence, HistoryTracking, WorkDirPersisted, SameErrorTrip, ListSessions, PromptInstructions.
- **test/commands/loop_test.go** (Command-level): Tests cover Iterate_RequiresDocker (full wiring), Iterate_JSONOutput, Iterate_ContainerCleanup, Iterate_AgentNameAutoGenerated (runF trapdoor), Tasks_RequiresTasksFlag, Tasks_TasksFileResolution, Tasks_MutualExclusivity, Status_NoSession, Shared_SetupLoopContainer (lifecycle), Shared_ConcurrencyDetection, Iterate_ConfigOverrides, Iterate_ExplicitFlagWins, Iterate_PromptMutualExclusivity, Iterate_RequiresPromptSource.
- **Key testing pattern**: Injected fake `claude` shell scripts into containers that output LOOP_STATUS blocks. Scripts use counter files (`/tmp/loop-counter`) to produce different output on successive invocations (e.g., "signal completion on 2nd call"). This avoids needing a real Claude installation while exercising the full exec → output → parse → circuit-breaker pipeline.
- **Helper functions added**: `createLoopTestContainer` (shared container setup), `newTestRunner` (temp-dir stores), `writeScriptToContainer` (exec-based file injection), `injectLoopHooksForTest` (test-side hook injection using `loop.ResolveHooks`), `newCopyFn` (wraps docker client), `buildTestSettingsTar`/`buildTestHookFilesTar` (tar builders for CopyToContainer).
- All 4009 existing unit tests still pass. Both test packages compile cleanly with `go vet` and `go build`.
