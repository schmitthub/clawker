...
| Task 5: Protocol migration — proto rewrite + handler rewrite to 13-method scope-corrected surface | `complete` | opus |
| Task 6: Factory `f.AdminClient` + command migration + delete `firewall.Manager` | `complete` | opus |
| Task 7: Break-glass `clawker controlplane up/down/status` CLI | `complete` | opus |
| Task 8: Delete `internal/firewall/` + final cleanup + docs | `complete` | opus |

## Key Learnings

- **Task 8 (Opus, 2026-04-14):** Final cleanup. `internal/firewall/` was already gone post-Task-6, so the work was pruning unused support symbols + rewriting docs. Deleted: `config.Config.FirewallPIDFilePath()` + `FirewallLogFilePath()` accessors (impls in `internal/config/consts.go`); `consts.FirewallPIDFile`/`FirewallLogFile` constants; `consts.FirewallPIDPath`/`FirewallLogPath` helpers; `consts.PidsDir` (reviewer-flagged, zero callers). Regenerated `internal/config/mocks/config_mock.go`. `test/e2e/harness/harness.go:101` dropped the dead `"firewall.log"` iteration — CP writes `consts.ControlPlaneLogFile` only. `PurposeFirewall` label value deliberately KEPT (Envoy/CoreDNS containers still use it via `firewall.Stack.containerSpec`). `FirewallDataSubdir`/`FirewallCertSubdir`/`EgressRulesFileName`/`RequiredFirewallDomains`/`RequiredFirewallRules` all survive (live callers). Comment path fixes in `cmd/coredns-clawker/main.go` + `internal/controlplane/firewall/ebpf/types.go`. User-facing docs (`docs/*.mdx`, `README.md`, `docs/cli-reference/`) intentionally NOT updated per user direction — they're not published yet; update at release time. comment-analyzer reviewer caught three errors in the rewritten `internal/controlplane/firewall/CLAUDE.md` that would have shipped: (1) "Handler caches cgroupDriver at construction" was wrong — driver lives in the `containerResolverFromDocker` closure in `cmd/clawker-cp/main.go`, not on Handler; (2) Gotcha "re-enforce cost after bypass expires is a full re-enroll" was backward — `bypassTimerFired` (`handler.go:549`) calls `h.ebpf.Enable(enableID)` (cheap clear-bypass path), NOT `Install`. Full re-enroll only on the explicit `FirewallEnable` RPC. Task 5 learnings had this backward too (legacy-package shim delegated to `InstallFirewall`, but that died with Task 6); (3) "DomainHash shared across three packages" conflated `normalizeDomain` (string normalization) with `DomainHash` (FNV-1a). All three fixed. Gates: build/vet/make test (4625 pass) green. `check-claude-freshness.sh` single warning is pre-existing `internal/logger/CLAUDE.md` (unrelated). All six review subagents invoked (code-reviewer / silent-failure-hunter / test-hunter / code-simplifier / comment-analyzer / type-design-analyzer); findings addressed.

- **Task 7 (Opus, 2026-04-14):** Break-glass `clawker controlplane up/down/status` CLI shipped. First attempt used package-level test seams (`ensureRunning`, `stopFn`, `cpRunningFn`, etc.) — user rejected as "hard-coded seams". Correct design: new `controlplane.Manager` interface in `internal/controlplane/manager.go` with four methods — `EnsureRunning`, `Stop`, `IsRunning`, `ProbeHealthz`. Concrete impl wraps `func() (*docker.Client, error)` / `func() (config.Config, error)` / `func() (*logger.Logger, error)` lazy closures so the Manager re-resolves each dep per-method and a CLI path that never touches the CP never resolves Docker/Config/Logger. Factory gains `ControlPlane func() controlplane.Manager`; `controlPlaneFunc(f)` in `internal/cmd/factory/default.go` is `sync.Once`-cached. moq mock at `internal/controlplane/mocks/manager_mock.go` via `//go:generate go run github.com/matryer/moq@v0.5.3 -out mocks/manager_mock.go -pkg mocks . Manager`. Tests drive `mocks.ManagerMock` directly; unprogrammed `*Func` fields panic on invocation and that's the "must not be called" negative assertion. `statusRow` grew a `FirewallError` field so AdminClient dial errors and RPC errors surface in `--json` output (previously silently dropped). Semantic colors (`cs.Success`/`cs.Error`/`cs.Info`) throughout — no raw `cs.Red`/`cs.Green`. Harness `FactoryOptions.ControlPlane` defaults to `&cpmocks.ManagerMock{}`. E2E at `test/e2e/controlplane_cli_test.go` authored (not run) — walks `up → up (idempotent) → status → down → status` + no-op `down` on absent CP. All six Factory-noun / test-seam lessons land in `internal/cmd/controlplane/CLAUDE.md`.

- **Task 6 (Opus, 2026-04-14):** Entire `internal/firewall/` package deleted (manager.go/daemon.go/manager_network.go/firewall.go/mocks/ + CLAUDE.md). `f.Firewall` swapped for `f.AdminClient` across Factory, 13 firewall subcommands, container start/stop/run/restart/remove, loop iterate/tasks, shared.CommandOpts, and e2e harness. `adminClientFunc` in `internal/cmd/factory/default.go` caches `grpc.ClientConn`; rebuilds only on `TransientFailure`/`Shutdown`; keepalive `30s/10s/PermitWithoutStream=false`. Test seam: `var ensureRunning = controlplane.EnsureRunning`. `auth.DialCPAdmin` extended to accept variadic `grpc.DialOption` so the closure can inject keepalive without touching the TLS/OAuth2 baseline. `ProjectRules(cfg)` moved from `internal/firewall/manager.go` into `internal/controlplane/firewall/rules_store.go`. `protoRulesToConfig`/`configRulesToProto` exported as `ProtoRulesToConfig`/`ConfigRulesToProto` for `BootstrapServicesPostStart` to reuse. `BootstrapServicesPostStart` now makes three RPCs: `FirewallInit` (idempotent stack-up) → `FirewallAddRules` (project rules, sync) → `FirewallEnable` (drift-guarded per-container enroll) — PID-file daemon path is gone. AdminServiceClient mock generated into `internal/controlplane/mocks/admin_client_mock.go` via `//go:generate` directive on `api/admin/v1/admin.go` (relative path: `../../../internal/controlplane/mocks/`). Test-hunter follow-up executed: `internal/controlplane/firewall/rules_store_test.go` trimmed to `TestValidateDst` + `TestEgressRulesFileFields_AllFieldsHaveDescriptions`; all `TestAddRules_*` / `TestRemoveRules` deleted (covered by E2E); `fwlegacy` alias import dropped. `down.go` test suite drastically simplified — no more PID file, `IsDaemonRunning`, `StopDaemon`, `WaitForDaemonExit`. `make test` green: 4607 tests pass. Gotcha: pre-existing config mock drift on `CPIPLastOctet` — resolved by running `go generate ./internal/config/...` to refresh moq output.

- **Task 5 (Opus, 2026-04-14):** 13-method proto surface landed; per-container RPCs lost the `cgroup_path` field. Handler resolves cgroup paths internally via cached `cgroupDriver` + `EBPFCgroupPath` + Docker-backed `ContainerResolver` (signature `(ctx, ref) → (canonicalID, cgroupPath, exists, err)` — exists=false + err=nil is the "container gone" signal). Drift guard (INV-B2-016) lives in two places: `(h *Handler).resolveForEnable` for direct Enable, `firewall.resolveBypassCgroupID` for the bypass dead-man timer; both use the same resolver tuple, both Wlog stored-vs-fresh deltas. Bypass also seeds `storedCgroupID[cid]` so a Disable issued mid-bypass on a now-gone container can still clear the orphan bypass_map entry via the fallback branch.
- **Task 5 (Opus):** Implementation deviates from spec §8 prose for FirewallDisable: spec says "remove from container_map", we use the existing `mgr.Disable(cgroupID)` which sets bypass_map. Net BPF outcome is identical (fast path exits to bypass either way) and the choice avoided adding a new RPC to `EBPFManager` interface. Documented in handler doc comment + CLAUDE.md row.
- **Task 5 (Opus):** `EBPFManager` interface gained `FlushAll() error` so Handler.FirewallRemove can drive global teardown through the same mock seam used for per-container RPCs. moq mock regenerated in same change.
- **Task 5 (Opus):** `controlplane.adminServer` composite struct embeds `*fwhandler.Handler` so future domain handlers (monitor, hostproxy, clawkerd) plug in by being embedded alongside. **Embedding gotcha:** removed the redundant `adminv1.UnimplementedAdminServiceServer` from the composite — `Handler` already embeds it, and Go rejects two embedded fields contributing the same method at the same depth (ambiguous selector). When adding the second domain handler, only one of them may embed `Unimplemented...Server`.
- **Task 5 (Opus):** `AdminMethodScopes()` moved from `internal/controlplane/hydra_client.go` to `api/admin/v1/admin.go` so it lives next to the generated bindings. Adding a new RPC without a scope entry now fails closed via `TestAdminMethodScopes_CoversAllRPCs` reflecting over `AdminService_ServiceDesc`. `internal/consts.ScopeAdmin` import lifted into `api/admin/v1/`; no cycle (consts has no api/ deps).
- **Task 5 (Opus):** Legacy `Manager.Enable(containerID)` shim now delegates to `InstallFirewall` instead of issuing a cheap `cpClient.Enable` — there is no longer a B1 "clear bypass" RPC. Means callers that previously paid only for a single RPC now pay the full re-enroll cost. Acceptable for the 1-2 task transition window (legacy package is deleted in Task 6/8); flagged in silent-failure-hunter pass.
- **Task 5 (Opus):** Deleted `internal/firewall/manager_test.go` (672 lines) outright — its `recordingAdminClient` impl'd the now-stale 7-method `adminv1.AdminServiceClient` interface, and most tests asserted on `cgroup_path` request fields the proto rewrite removed. `cgroup_test.go` in the new subpackage already covers `ResolveContainerID` / cgroup-driver branches; `daemon_test.go` (no adminv1 deps) stays. The legacy adapter shim is small enough that the build + a single unit test failure in CLI command code would catch any regression in the 1-2 task window before Task 6/8 deletes the package entirely.
- **Task 5 (Opus):** PathRule field name in proto switched from spec's `prefix` to `path` to match `config.PathRule.Path` exactly. Avoids a translation mapper between the wire format and the on-disk schema.
- **Task 5 (Opus):** `cmd/clawker-cp/main.go::containerResolverFromDocker` preserves the canonical-64-hex ref as the `cid` even on Docker NotFound, so `FirewallDisable`'s stored-cgroup-id fallback can still key on it. Reuses `firewall.IsCanonicalContainerID` (newly exported) for symmetry with the in-package validator — no duplicate definitions.
- **Task 5 (Opus) — review subagents applied:** silent-failure-hunter caught the Bypass-doesn't-seed-storedCgroupID gap (fixed); code-reviewer caught the non-canonical short-ref NotFound silently dropping enforcement state (fixed via `IsCanonicalContainerID` export); code-reviewer + simplifier caught `noopEBPF()` in `authz_test.go` missing `FlushAllFunc` (fixed); test-hunter flagged duplicate `TestHandler_FirewallSyncRoutes_*` tests (deleted; covered by `TestAdminHandler_SyncRoutes` in `authz_test.go`) and the never-invoked `var _ = func()` compile guard (deleted) and `TestProtoRulesToConfig_ZeroValuesAllowed` (deleted as tautological); comment-analyzer caught FirewallDisable doc inaccuracy (says "removes from routing" but uses bypass_map — rewritten), `CancelAllBypassTimers` race comment understatement (rewritten), drift.go case-2 comment misleading about who clears (rewritten), `admin.proto::FirewallInit` doc claiming BPF attach happens during the RPC when it actually happens at CP startup (rewritten); simplifier caught fakeStack's 5 unused fields (dropped) and `addRulesToStore`'s missing pre-allocation (fixed). Type-design audit's larger refactors (ContainerResolver tuple → struct, driftCache abstraction, HandlerDeps split into CoreDeps+StackDeps) were deferred — they are good directions but out of scope for Task 5's fix-only review pass.
- **Task 5 (Opus) — kept for Task 6/8 cleanup:** `internal/firewall/manager.go` `Enable` shim delegating to full `InstallFirewall` is a behavioral change vs B1. The legacy CLI-side cgroupDriver/ebpfCgroupPath helpers in `manager.go` remain as dead-but-compiled code paths — they belong to the `Manager` struct that Task 6 deletes wholesale.
