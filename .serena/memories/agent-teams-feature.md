...
3. Credentials from being forwarded
...
- **Credentials**: `.credentials.json` contains OAuth tokens. Full access by container (read-write). Acceptable because container already has code access and network access
- **Bidirectional writes**: Container can modify all host Claude config. This is the desired behavior (read-write requirement)
