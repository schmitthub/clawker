...
```bash
./bin/clawker --debug run      # Start with debug logging
./bin/clawker init               # Initialize project
./bin/clawker run --remove        # Start ephemeral container (random agent name)
./bin/clawker run --agent ralph  # Start with named agent
./bin/clawker run --agent ralph --shell -s /bin/zsh/ --user claude # run shell with specific user
./bin/clawker run --remove mcp list       # Run ephemeral claude command
