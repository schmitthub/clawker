...
> **STATUS 2026-07-17 — migration STARTED (branch feat/plugin).** Constraints 2–3 below are RESOLVED by a submodule model the user chose: repo `schmitthub/claude-plugins` renamed → `schmitthub/clawker-plugin`, plugin content migrated there root-as-plugin (marketplace source = relative `"./"`, update-plugin.yml deleted, v1.0.24 @ `19dfbba`), and mounted back into clawker as the `clawker-plugin/` git submodule (clawker `81cf240c`) — drift enforcement now runs against the submodule checkout, the pointer replaces the sha pin, and the dir rename happened as part of this (old `claude-plugin/` gone). Copy lane learned the relative-source shape and stays as the interim codex/opencode/pi path. Design options 2–4 below (opencode lane, codex UAT, native-dispatch wrappers) remain OPEN for Phase C. Current state of record: auto-memory `project_plugin_migration_submodule`.
...
Related: `mem:plans/multi-harness/harness-research-opencode-pi` (install/config/auth recon for the harness bundles themselves), auto-memory `project_multi_harness_initiative` (branch state + mandate verbatim).
