...
Cross-references: `feedback_parity_needs_value` (record absent from this checkout), `feedback_no_invented_security_framings` (record absent from this checkout).
...
## Earlier agent errors (avoid repeats — see `feedback_no_invented_security_framings` (record absent from this checkout))
...
### Phase 5 — Resume paused UAT (separate concern, see `dockerfile_cache_optimization_uat` (record absent from this checkout))
...
- **DELETE THIS MEMORY when work complete.** Once Phase 3/4/5 done, ask user to delete via `mcp__serena__delete_memory` with name `mem:plans/docker-build-parity`. Captures in-flight investigation state, not durable project facts.
