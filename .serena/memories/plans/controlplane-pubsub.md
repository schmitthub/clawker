...
Test placement (2026-09-07): `internal/controlplane` is the CP entrypoint package. Keep helper tests in `cmd_helpers_test.go`; do not create separate files for SDS helpers there. `TestStopSDSServer`, `TestRequireSDSClientSAN`, and their helpers moved into that file. The two `sds_*_internal_test.go` files were removed. SDS service tests belong beside the service in `controlplane/firewall/`.

Follow-up (2026-09-07): Removed all 20 original PR-added lint suppression directives and initialized required struct fields explicitly. The user then approved only eight staticcheck directives on deprecated TLS fields in `sdsTLSConfig` and `newSDSStopTestServer`. No exhaustruct or gosec suppression is approved. `HostFirewallSDSCertsDir` is now an accessor. SDS file ownership now matches the explicit Envoy UID/GID, with directory mode 0750 and file mode 0600. Local unit tests do not prove access across the actual CP and Envoy containers. The user ran both wildcard E2E tests on the host. The SDS test passed first; the coexistence test exposed a missing CP removal step in shared test setup. Moving `EnsureNoControlPlane` into `fwSetup` fixed test isolation, and the host rerun passed both tests. Do not add substitute file-mode tests or run further Docker probes. See `mem:history/pr-519-checks-and-approvals` for the final check status.
