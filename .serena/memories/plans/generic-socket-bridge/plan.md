...
Authority: `mem:plans/generic-socket-bridge/requirements` (24 ratified decisions).
...
5. **Per-task exit gate:** before moving on you MUST edit THIS file
   (`.serena/memories/plans/generic-socket-bridge/plan.md`): set the
...
- Update `.claude/docs/ARCHITECTURE.md` / `KEY-CONCEPTS.md` if they enumerate
  subsystems.
- Update serena memory `plans/generic-socket-bridge/requirements` status line and
