...
| Task 3: CP shutdown integration + E2E tests | `complete` | Opus 4.6 |
...
- **CP drain wiring landed in Task 2's commit, not Task 3.** Initiative plan assumed Task 3 would wire `actionQueue.Close()` into `cmd/clawker-cp/main.go::drainCallback`. In practice it was landed alongside Task 2 (the handler refactor) because Task 2 couldn't compile without `HandlerDeps.Queue` being wired from main. Task 3 verification confirmed the drain sequence matches INV-B2-007 new order: `actionQueue.Close` → `grpcServer.GracefulStop` → `handler.CancelAllBypassTimers` → `stack.Stop` → `ebpfMgr.FlushAll`, with `errors.Join` aggregating teardown failures. No further wiring was needed.
- **No new E2E tests written — unit coverage proved the invariants.** The initiative plan listed ~7 E2E scenarios (coalescing-via-restart-count, teardown preserves rules, post-teardown rule removal takes effect, CLI remediation hints, etc.). Per user guidance "do not write any new tests whose paths are already covered, even if inadvertently, by other tests." Handler unit tests (`TestHandler_FirewallRemove_PreservesRulesStore`, `TestHandler_AddRules_StackDown_PersistsWithoutRestart`, `TestHandler_AddRules_InvalidDomain_ReturnsRuleInvalid`, `TestHandler_Reload_CallsSyncRoutesAfterReload`) + queue unit tests (FIFO, coalescing matrix, panic recovery, Close-then-Submit race, drain-on-Close) cover every load-bearing invariant. Existing E2E (`TestFirewall_AddRemove` through CLI, `TestFirewall_ConfigRules` with concurrent mutations) exercises the RPC→Queue→Handler→Stack integration without duplication. Restart-count assertions for coalescing would be flaky and add no signal unit tests don't already provide.
- **Close must cancel ctx BEFORE broadcast, inside the locked region.** Original T1 implementation called `q.cancel()` after `q.mu.Unlock()`. That left a race window where a worker woken by Broadcast could pop an item and begin executing `item.fn(q.ctx)` with a still-live context, violating the documented contract that drained closures observe a cancelled ctx. Fix moves `q.cancel()` inside the locked region before `q.cond.Broadcast()` so the ctx is already done by the time the worker acquires the lock in `next()`. Surfaced by silent-failure-hunter during T3 review.
- **`toStatus(err)` for every queue-submit error path, not `status.Errorf(codes.Internal, ...)`.** T2 wrote three per-container handlers (Enable, Disable, Bypass) with `if errors.Is(err, ErrQueueClosed) { return nil, toStatus(err) } else { return nil, status.Errorf(codes.Internal, "enable: %v", err) }`. That stripped any future non-ErrQueueClosed sentinel the closure could add. T3 review flagged it; fix is one unconditional `return nil, toStatus(err)` — `toStatus` already handles both sentinel-wrapped and bare errors. Symmetric lesson: favor one code path over branching error handling — the branch is silent-failure bait when sentinel catalogs grow.
- **9 CLI call sites were bypassing `wrapRPCError` / `callWithSpinner`.** T2 added both helpers and wired `add`/`remove`/`reload`/`rotate-ca`. The other 8 commands (`up`, `down`, `status`, `list`, `enable`, `disable`, and 3 paths in `bypass`) kept their old `fmt.Errorf("starting firewall: %w", err)` wrapping, which meant errdetails.ErrorInfo entries never reached `remediationLines` and the user never saw remediation hints. T3 review surfaced this; fix is mechanical — wrap the RPC body with `callWithSpinner(ctx, ios, label, fn)` + error wrap with `wrapRPCError(header, err)`. `callWithSpinner` also enforces the 15s RPC timeout (otherwise a stuck CP hangs the CLI indefinitely for reads like `firewall list`). Two bypass cleanup paths (post-interactive Ctrl+C Enable + post-timer-expire Enable) keep their explicit 10s context.WithTimeout and the manual `adminClient.FirewallEnable` call because they need a fresh Factory-rebuilt grpc.ClientConn — but their error path still goes through `wrapRPCError`.
- **Two unit tests were pure waste; one was a merge candidate.** `TestActionQueue_CloseIsIdempotent` was structurally obvious — second Close deadlocks on already-closed `q.done`, caught by any other Close-based test. `TestActionQueue_ConcurrentSubmitters` exercised "128 goroutines submit, assert no error" without any ordering/coalescing/race assertion — `TestActionQueue_CloseThenSubmitRace` already exercises concurrent Submit under the harder condition. `TestActionQueue_CoalescesConsecutiveReconciles` and `TestActionQueue_ResultPropagatesToAllCoalesced` both exercised the same head-wins coalescing contract on overlapping code paths — merged into one table-driven `TestActionQueue_CoalescedResultBroadcast` that asserts both the "runs once" drop guarantee AND the "every submitter gets HEAD's Result" fan-out guarantee, with success/failure sub-cases. Net reduction: 3 tests → 1; signal preserved.
