...
func TestContainerRun_Labels(t *testing.T) {
	...
	require.Equal(t, "true", labels[_blankCfg.LabelManaged()])
	require.Equal(t, "run-label-test", labels[_blankCfg.LabelProject()])
	require.Equal(t, agentName, labels[_blankCfg.LabelAgent()])
...
func TestContainerRun_ContainerNameResolution(t *testing.T) {
	...
	require.Equal(t, "run-name-test", labels[_blankCfg.LabelProject()], "project label mismatch")
	require.Equal(t, agentName, labels[_blankCfg.LabelAgent()], "agent label mismatch")
...
// TestContainerRun_InvalidAgentName tests that invalid agent names are rejected
...
func TestContainerRun_InvalidAgentName(t *testing.T) {
	harness.RequireDocker(t)
	ctx := context.Background()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			client := harness.NewTestClient(t)
			defer func() {
				_ = harness.CleanupProjectResources(context.Background(), client, "run-invalid-test")
			}()

			f, _ := harness.NewTestFactory(t, h)
			cmd := run.NewCmdRun(f, nil)
			cmd.SetArgs([]string{"--detach", tt.flag, tt.agent, "alpine:latest"})
			...
			containers, listErr := client.ListContainersByProject(ctx, "run-invalid-test", true)
