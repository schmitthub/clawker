...
// runRealFirewall installs dependencies and runs the real init-firewall.sh inside a container.
// This ensures tests exercise the actual production script rather than hardcoded inline copies
// that can drift out of sync.
func runRealFirewall(t *testing.T, ctx context.Context, ctr *harness.RunningContainer, client *docker.Client) *harness.ExecResult {
	t.Helper()

	// Install all deps needed by the real script
...
func TestFirewall_IptablesDropPolicy(t *testing.T) {
	...
	execResult := runRealFirewall(t, ctx, ctr, client)
...
func TestFirewall_BlockedDomainsUnreachable(t *testing.T) {
	...
	// Try to curl a blocked domain (should fail — not in allowed domains)
