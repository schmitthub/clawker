...
func setup(ctx context.Context, cli *client.Client) error {
	...
	// Create test engine
	testEngine, err = NewEngine(ctx, EngineOptions{
...
func TestImageList_OnlyReturnsManagedImages(t *testing.T) {
	ctx := context.Background()
	...
	if !foundManaged {
		t.Error("ImageList should return managed image")
	}
	if foundUnmanaged {
		t.Error("ImageList should NOT return unmanaged image")
	}
}
