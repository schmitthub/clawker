...
- `toSolveOpt(opts)` — converts `ImageBuildKitOptions` to `bkclient.SolveOpt`; uses "moby" exporter, "dockerfile.v0" frontend. A relative `Dockerfile` resolves inside the context mount; an absolute one gets its own "dockerfile" local mount rooted at its directory, with the frontend seeing only the base name. When `NoCache=true`, sets both `no-cache` frontend attribute AND empty `CacheImports` (per moby/buildkit#2409, the attribute alone only verifies cache rather than disabling it)
- `drainProgress(ch, onProgress)` — reads `SolveStatus` channel; when `onProgress != nil`, converts vertices to `BuildProgressEvent` with state transition deduplication and forwards log lines (keeping only the text after the last `\r` from build tool progress bars, then stripping ANSI escape sequences so build output cannot inject terminal escapes). A nil callback drains silently
...
- **BuildKit**: `FakeBuildKitBuilder(capture)` with `BuildKitCapture{Opts, CallCount, Err, ProgressEvents, RecordedEvents, DelayMultiplier}` — when `ProgressEvents` is set and `OnProgress` callback provided, emits events before returning. `FakeTimedBuildKitBuilder(capture)` — same but sleeps `RecordedEvents[i].Delay()` (scaled by `DelayMultiplier`; 0 or 1 = unscaled) between events for realistic replay timing
