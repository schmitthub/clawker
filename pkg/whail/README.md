...
Wraps `github.com/moby/moby/client`. **Only `pkg/whail` imports the moby `APIClient` connector, and only `internal/docker` imports `pkg/whail`.** Typed moby imports are fine anywhere; standalone daemon processes have narrow connector exceptions — see `.serena/memories/cli/core.md`.
