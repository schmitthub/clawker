...
- **Firewall: one domain per TCP/SSH port.** Raw TCP/SSH has no domain metadata (unlike TLS and HTTP where Envoy does full HTTP inspection), so eBPF routes traffic per-domain on a port to a single whitelisted domain. Two SSH hosts on port 22 isn't supported yet — first rule wins. Deferred for after the control plane work ([#235](https://github.com/schmitthub/clawker/issues/235))
