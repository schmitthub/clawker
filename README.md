...
- **Approved host socket bridges**: a harness can declare a required or optional Unix socket, and clawker shows the exact request and listener identity before it grants access. Stored decisions can be reviewed and revoked with `clawker sockets`; see [Host socket bridges](https://docs.clawker.dev/host-sockets)
