...
Why run a coding-agent CLI (Claude Code, Codex, …) inside clawker instead of on bare metal with the CLI's own sandboxing — or inside another tool built for the same job. Scope is limited to **sandboxes you run a vendored agent CLI in** (and the CLIs' own built-in sandboxes); programmatic sandbox SDKs and services for executing AI-generated code are separate categories and excluded. Every cell is a yes/no fact assessed against the vendor's **official documentation** (2026-07; cited notes in-repo), cross-checked by three independent passes. ✅ = the **sandbox itself** implements the capability — never something merely *possible* because it's a Linux box, and never a harness/CLI feature (an agent's own permission loop, worktree fan-out, or device-code login) that happens to run inside it. Hover any ✅ for its mechanism. ❌ = absent **or undocumented**. Wide table — scroll horizontally.
...
Assessed against official documentation only, as of 2026-07, reconciled across three independent assessment passes. Scope: tools you run a vendored coding-agent CLI inside, plus the CLIs' own built-in sandboxing. Programmatic sandbox SDKs (build-your-own security) and services for executing AI-generated code are different categories and are excluded from the table; research notes for them remain in-repo. Each column is a single yes/no fact; a criterion that resisted a clean yes/no was split into narrower columns rather than recorded as a middle value. A ✅ means the **sandbox product itself** implements the capability — not something merely *possible* because the sandbox is a general-purpose machine, and not a feature of the coding-agent harness/CLI running inside it. ❌ covers both confirmed absence and undocumented capability. Per-provider research notes with per-claim citations live in `.serena/memories/agent-sandbox-research/`.

- **Observability is three columns, not one.** `Metrics dashboard` = a real resource/metrics stack (not a status list). `Harness telemetry capture` = the sandbox ingests and surfaces the *harness's* own signals — for Claude Code: prompts, MCPs, skills, slash-commands, tool decisions/outcomes, cost, model breakdown. `Extensible monitoring` = a pluggable pipeline you extend for any signal. A harness emitting its own OTel is not the sandbox capturing it.
- **Active supervision** is an *external, sandbox-layer* supervisor that observes the running agent and can **contain** it. Being able to stop a process is not this — every non-broken program is stoppable — and a harness pausing to ask permission is the harness's loop, not sandbox oversight.
- **Domain-native vs IP-pinned**: `Domain-native` is ✅ only when rules match the hostname at request time (DNS-layer policy, SNI/Host matching, dynamic forward proxy). Resolving domains to IP sets once and enforcing as CIDR is ❌ — it breaks when load-balanced services rotate IPs and over-opens shared CDN/Cloudflare ranges.
