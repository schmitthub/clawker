...
| `progress.go` | User-facing TTY progress reporter: two plain status lines per init step (Active form, then ✓/✗ Done) plus boot/closing banners. No animation — per-step shell scripts complete in milliseconds, below the threshold animation would be perceptible. Writes to `os.Stdout` (the attached TTY for the agent container) via TIOCGPGRP-detected isTTY which only toggles ANSI color codes; line shape is identical for piped/non-TTY out. Wired by `main.go` → `startClawkerdListener` → `clawkerdServer` → `runSession` → `session`. Init step boundaries hooked in `session.dispatch` (Command_Shell with `init-` prefix → StartStep) and `session.send` (terminal Done/Error → EndStep); `handleAgentReady` calls `Final` immediately before spawn so the user CMD takes the TTY foreground without visual collision. |
