...
type Handler struct {
    ...
    Emitter Emitter  // shared per-process; ServeDNS nil-checks before calling Emit
}
...
const (
    pluginName            = "otel"
    envEndpoint           = consts.EnvCoreDNSOtelEndpoint // "CLAWKER_COREDNS_OTEL_ENDPOINT"
...
- Empty endpoint → `noopEmitter`, with a Warning naming the env var. This path is never cached (see Retry on failure below), so the Warning fires on every `setup` call — once per zone in the Corefile, not once at boot. The plugin still installs in every server block; `Handler.Emit` is a no-op so the directive stays valid Corefile syntax even when telemetry is disabled.
...
- SDK-side errors flow through `otel.SetErrorHandler` → `handleOTELError` → CoreDNS Error log (`log.Errorf`) as a structured `event=coredns_otel_unavailable component=coredns_otel_plugin cause=%q impact=security_event_loss` line, rate-limited to one per minute via `otelErrorRateLimit`; they never crash the plugin (consistent with the CP no-panic invariant — see root `CLAUDE.md`).
...
Additional coverage beyond the `Emitter`-seam tests above:

- `TestSetup_RejectsCorefileArgs` — pins the Corefile-arg-rejection contract in `setup`.
- `TestBuildTLSConfig` — table of `buildTLSConfig` error paths (empty paths, bad keypair, unreadable CA, CA with no PEM blocks), against self-signed test certs.
- `TestBuildTLSConfig_GetClientCertificateRotates` — confirms `GetClientCertificate` re-reads the leaf from disk on each call (not a cached `tls.Config.Certificates`), by rewriting the cert/key files mid-test.
- `TestEnsureSharedEmitter_EmptyEndpoint_NotCached` — pins that the empty-endpoint `noopEmitter` path is never cached in `sharedEmitter`.
...
- `github.com/schmitthub/clawker/internal/consts` (for `consts.EnvCoreDNSOtelEndpoint`)
...
- The plugin is process-scoped, not zone-scoped, but a misconfigured `CLAWKER_COREDNS_OTEL_ENDPOINT` does NOT produce a single boot-time Warning — the empty-endpoint path is never cached (`ensureSharedEmitter`), so every zone's `setup` call re-warns. Operators reading per-zone Corefile blocks may think otel is wired but the noopEmitter is doing nothing. Watch for the repeated `OTEL endpoint not configured` log line, one per zone.
