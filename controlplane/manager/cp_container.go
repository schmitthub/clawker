...
func BuildCPContainerConfig(cfg config.Config, opts CPContainerOpts) (*CPContainerConfig, error) {
	...
	// The log file path is <stateDir>/logs/clawkercp.log — mount the parent dir.
...
// cmd/clawkercp/clawkercp.go::otelOptionsFromEnv) and the collector's
...
func otelLogsEnv(cfg config.Config) []string {
	...
	return []string{
		...
		//
		// service.name=clawkercp is consumed by the OTel SDK's
		...
		// by this sender-declared name to the clawkercp index. Removing
		...
		// `component=clawkercp` are intentionally NOT stamped — they
		...
		"OTEL_RESOURCE_ATTRIBUTES=service.name=clawkercp",
	}
}
