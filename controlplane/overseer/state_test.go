...
// TestExec_WithStep covers the StepIndex clamp and the (name, index)
// pair contract subscribers rely on.
func TestExec_WithStep(t *testing.T) {
	cases := []struct {
		name        string
		stepCount   int
		idx         int
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			i := ExecRunning(tc.stepCount, time.Now()).WithStep("git", tc.idx)
...
// TestExecRunning_NegativeStepCountClamped pins the negative-count
// clamp at construction so a stale-event projection can't display
// "step N of -3" to subscribers.
func TestExecRunning_NegativeStepCountClamped(t *testing.T) {
	i := ExecRunning(-3, time.Now())
	assert.Equal(t, 0, i.StepCount())
}

// TestExec_Complete_ClearsLastError pins the success-snapshot
...
func TestExec_Complete_ClearsLastError(t *testing.T) {
	start := time.Date(2026, 5, 8, 12, 0, 0, 0, time.UTC)
	end := start.Add(time.Second)
	i := ExecRunning(3, start).WithStepError("stale error from a previous step").Complete(end)
	assert.Equal(t, ExecStatusCompleted, i.Status())
	assert.Equal(t, end, i.CompletedAt())
	assert.Empty(t, i.LastError())
}

// TestExec_TerminalAtFloors verifies both Complete and Fail floor
...
func TestExec_TerminalAtFloors(t *testing.T) {
	start := time.Date(2026, 5, 8, 12, 0, 0, 0, time.UTC)
	earlier := start.Add(-time.Hour)
	t.Run("complete", func(t *testing.T) {
		i := ExecRunning(3, start).Complete(earlier)
		assert.Equal(t, start, i.CompletedAt())
	})
	t.Run("fail", func(t *testing.T) {
		i := ExecRunning(3, start).Fail(earlier, "boom")
		assert.Equal(t, start, i.CompletedAt())
	})
}

// TestExec_Fail pins the failed-state shape: Status, CompletedAt, and
...
func TestExec_Fail(t *testing.T) {
	start := time.Date(2026, 5, 8, 12, 0, 0, 0, time.UTC)
	end := start.Add(time.Second)
	i := ExecRunning(3, start).Fail(end, "transport error")
	assert.Equal(t, ExecStatusFailed, i.Status())
	assert.Equal(t, end, i.CompletedAt())
