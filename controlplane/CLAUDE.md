...
9. STARTUP FLOW — ordered side-effect steps, each a pre-`SetReady` gate (exit 1, no eBPF flush, enrolled agents stay fail-closed):
   ...
   - `ebpfLoadFlow` (a `ControlPlane` method — the recovery protocol is orchestrator state management) — `ebpfMgr.Load()` + `CleanupStaleBypass` (INV-B2-013). The permission-denied arm is the ONE recoverable failure (rootless Docker): instead of exiting, it publishes ONCE on the orchestrator's recovery queue (`PublishRecovery` — delivered to every connected `WatchSOS` stream, and to any that connects later), retries the load every `recoveryRetryInterval`, and gives up only on ctx cancel or when no watcher stream is connected for `recoveryIdleTTL` (30s; a connected watcher holds the clock at zero — an abandoned CP shuts down rather than waiting forever). On success `ClearRecovery` ends every watcher stream cleanly. Every other load failure is unrecoverable and exits as before.
