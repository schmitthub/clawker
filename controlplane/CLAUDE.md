...
**Key mounts**: config dir (RO), CA cert (RO), CLI public JWK (RO), server TLS cert+key (RO), `FirewallDataSubdir` → `/var/lib/clawker/firewall` (RW — egress rules, Envoy/CoreDNS configs, MITM CA), `/sys/fs/cgroup` (RO), clawker's own BPF filesystem `BPFFSSubdir` → `/var/lib/clawker/bpffs` (RW, **rshared** propagation — the mount must travel out on a rootful host and in on a rootless one), logs dir.
