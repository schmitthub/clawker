...
9. STARTUP FLOW — ordered side-effect steps, each a pre-`SetReady` gate (exit 1, no eBPF flush, enrolled agents stay fail-closed):
   ...
   - `startSDSServer` — the firewall on-demand certificate SDS listener (Envoy ↔ CP, clawker-net only, `ControlPlaneSettings.SDSPort`). Serves per-SNI MITM leaves to Envoy's wildcard-chain certificate selector. NOT a gate: any failure degrades with `event=sds_unavailable` (wildcard multi-label handshakes fail closed; everything else stays up). Runs before the firewall gate so Envoy can fetch secrets from its first boot.
