...
func Dial(ctx context.Context, adminPort, hydraPort int, opts ...grpc.DialOption) (adminv1.AdminServiceClient, *grpc.ClientConn, error) {
	...
	target := fmt.Sprintf(consts.Localhost+":%d", adminPort)

	hydraTokenURL := fmt.Sprintf("https://"+consts.Localhost+":%d/oauth2/token", hydraPort)
...
// bearer token). The manager readiness gate (waitForCPClockSync) polls
...
func ProbeCPTime(ctx context.Context, adminPort int) (time.Time, error) {
	...
	return probeCPTime(ctx, fmt.Sprintf(consts.Localhost+":%d", adminPort), tlsCfg)
}
...
// UTC). Used by ProbeCPTime, which the manager readiness gate polls to wait
...
func fetchAccessToken(ctx context.Context, signingKey *ecdsa.PrivateKey, tokenURL string, tlsCfg *tls.Config) (string, time.Duration, error) {
	assertion, err := auth.BuildSignedAssertion(auth.AssertionClaims{
		...
		// truth. The manager readiness gate aligns the CP clock to the host when
