...
func (e *HealthTimeoutError) Error() string {
	return fmt.Sprintf(
		"%v after %s — this can happen during first run when firewall container images are being pulled, or may indicate a bug. Run 'docker ps -a --filter label=dev.clawker.purpose=firewall' to check container status, or try the command again",
		e.Err,
		e.Timeout,
	)
}
