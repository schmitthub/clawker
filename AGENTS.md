...
- Package dependency and layering questions: run `go list -deps ./<pkg>` or `go list -f '{{.ImportPath}} {{.Imports}}' ./...` and answer from that output. The graphify graph has no Go package import edges.
