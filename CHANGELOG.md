...
- **Changed:** Stack and harness registration is per-project in `clawker.yaml` (`stacks:` / `harnesses:`), not in `settings.yaml`. The former settings-side `stacks:`/`harnesses:` registry is removed and stripped from `settings.yaml` on load with a notice.
