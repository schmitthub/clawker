...
- **Added:** Language **stacks** (`build.stacks: [go, node, python, rust]`) — file-backed, user-editable install definitions registered in `settings.yaml` and materialized to `~/.config/clawker/stacks/`; harness bundles declare the stacks they need and projects add their own.
