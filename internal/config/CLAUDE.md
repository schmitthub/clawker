...
- **Project vs Settings scope** — Project keys: `build`, `agent`, `workspace`, `security`, `aliases`. Settings keys: `logging`, `monitoring`, `host_proxy`, `firewall`, `control_plane`, `docker`, `aliases`. Project identity (name) is resolved at runtime via `project.ProjectManager.CurrentProject(ctx).Name()`, not stored in config.
- **Two aliases keys, one active** — `Settings.Aliases` (union-merged, ships a default `dev` alias) is what the CLI registers as commands. `Project.Aliases` is a dormant sharing vehicle consumed only by `clawker alias import`/`export` — never applied automatically.
