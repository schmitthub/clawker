...
| `identity.go` | Project identity constants: `Domain`, `LabelDomain`, `ContainerUID`, `ContainerGID` |
| `ip_ranges.go` | IP range source registry, `GetIPRangeSources()` method |

## Constants

- **Identity (`identity.go`):** `Domain` (`clawker.dev`), `LabelDomain` (`dev.clawker`) — reverse-DNS for Docker/OCI labels. `ContainerUID` / `ContainerGID` (`1001`) — single source of truth for container user UID/GID, used by bundler, docker, containerfs, and test harness.
