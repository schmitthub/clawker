...
| `DockerHost()` | `docker.host` (`$DOCKER_HOST` wins, then the setting, then the active docker context, then `/var/run/docker.sock`; returned raw, no stripping, no validation) | `string` |
...
- **No env var overrides** — `CLAWKER_*` env vars affect only directory resolution (`CLAWKER_CONFIG_DIR`, etc.), not config values. The one deliberate exception: `DockerHost()` honors `$DOCKER_HOST` over `docker.host` — that variable is the docker CLI's own contract. It also reads the active docker context below the setting (`internal/docker/context`), which is the only place a stock rootless install records its daemon address. The value is a full address, scheme and all, returned raw — a caller needing a bind-mount source takes the path out of it itself.
