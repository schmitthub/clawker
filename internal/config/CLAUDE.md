...
| `migrations.go` | `ProjectMigrations()`, `SettingsMigrations()` — schema migration functions applied at load time, per file layer. Project chain (in order): legacy run-list → `[]string` conversion; strip of deleted `build.image`/`build.dockerfile`/`build.context`/`agent.claude_code.use_host_auth` keys (one-shot stderr notice naming each key + value + replacement); `agent.claude_code` → `harnesses.claude` rewrite (field-for-field move, or drop with a notice when a `harnesses.claude` entry already out-ranks it; the read shim in `schema.go` stays for unmigrated read-only contexts). Settings chain: legacy monitoring-key removal/rename. Registry seeding (`harnesses:`/`stacks:`) is deliberately NOT a migration — the build-time ensure paths in `internal/bundler` own all registry writes; config load never invents registry state |
| `validate.go` | `validateProjectRegistries(*storage.Store[Project]) error` — front-door validation for the `stacks:`/`harnesses:`/`build.harnesses:` nodes, called by `NewConfig`/`NewFromString`/`NewBlankConfig`/`NewProjectStoreFromPreset`. Walks each discovered layer (never the merged tree, so errors name the actual file) and rejects a bad registered/overlay name (unified naming rule, `internal/consts.ValidateName`/`ValidateHarnessName`), an unknown field under one of these nodes, a `harnesses.<name>.config.strategy` outside the copy/fresh vocabulary, or a registry `path` that is empty, uses `~`, or uses `$VAR` expansion. NOT invoked on the `ProjectStore().Set`/`Write` mutation path — a future write front-door (e.g. a stack/harness register command) must call it (or equivalent per-value checks) itself |
...
- **Unknown fields are silently accepted** by `NewFromString`/`NewConfig` — **except** under `stacks:`, `harnesses:`, and `build.harnesses:` (including its nested `inject:`), where `validate.go`'s front-door check rejects an unknown field as a load error naming the file and key path. This is a deliberate, narrower exception to the general rule below, not a project-wide strict-decode.
...
- **Stack/harness registry names fail the whole load, not just the field** — `NewConfig`/`NewFromString`/`NewBlankConfig` all call `validateProjectRegistries` after loading the project store; a `stacks:`/`harnesses:`/`build.harnesses:` key that fails `internal/consts.ValidateName`/`ValidateHarnessName` (not lowercase kebab-case, >32 chars, or — for harnesses — a reserved image-tag alias) or a registry `path` using `~`/`$VAR` returns a hard error from the constructor, not a partial/degraded `Config`.
