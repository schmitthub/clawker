...
// Dst is the domain or IP, Proto defaults to "http" (TLS-terminated HCM), Action defaults to "allow".
// The legacy value `proto: tls` is silently translated to `proto: http` at normalization time.
type EgressRule struct {
	...
	Proto       string     `yaml:"proto,omitempty" label:"Protocol" desc:"L7 protocol: http (TLS-terminated, MITM-inspected; default) or any opaque L7 name (e.g. ssh, redis) for TCP pass-through"`
	Port        int        `yaml:"port,omitempty" label:"Port" desc:"Override the default port (443 for http)"`
	Action      string     `yaml:"action,omitempty" label:"Action" desc:"Allow or deny traffic to this destination (default: allow)"`
	PathRules   []PathRule `yaml:"path_rules,omitempty" label:"Path Rules" desc:"Fine-grained path filtering (only applies when proto is http)"`
...
type FirewallConfig struct {
	AddDomains []string     `yaml:"add_domains,omitempty" merge:"union" label:"Firewall Domains" desc:"Shorthand: domains the container can reach over HTTPS (converted to http+port-443 rules)"`
