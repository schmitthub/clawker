...
**Host-UID is baked into the rendered Dockerfile.** `consts.ContainerUID()` / `ContainerGID()` resolve to the CLI invoker's `os.Getuid()` / `Getgid()`, so `useradd --uid {{.UID}}` in `Dockerfile.tmpl` varies per host user. The content hash therefore differs across hosts (and across users on the same host) even for identical project configs — multi-user hosts naturally get separate cached images, and rebuilds fire automatically when a different user runs `clawker run`. Required for the `~/.claude/projects` bind-mount writability contract; not a cache bug.
