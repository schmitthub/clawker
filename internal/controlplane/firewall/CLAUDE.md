...
| `envoy_config.go` | Envoy YAML generation; per-domain filter chains; LOGICAL_DNS clusters; TCP/SSH listeners; dual-sink access log builder (stdout JSON for `docker logs` triage + native `envoy.access_loggers.open_telemetry` OTLP/gRPC sink). The `otel_collector_als` cluster is parameterized by `ALSConfig` — when `MTLS=true` it dials the CP-only `otlp/infra` receiver on `OtelInfraPort` with an upstream TLS transport_socket (leaf+intermediate bind-mounted at `/etc/envoy/otel-tls/`, CLI root CA at `ca.pem`); otherwise plaintext to `otel-collector:4317`. `normalizeDomain` lives here — used by certs, coredns_config, rules_store, and shared with `internal/dnsbpf` via `ebpf.DomainHash` |
| `stack.go::ensureInfraClientCerts` | Mints short-lived (1y) mTLS client leaves for Envoy and CoreDNS via `InfraIssuer` (satisfied by `*infracerts.Issuer`). Writes `<FirewallOtelClientsDir>/<svc>/client.{pem,key}` + a shared `ca.pem` copy of the CLI root. Sibling Envoy/CoreDNS containers bind-mount the per-service paths. Runs inside `ensureConfigs` so `Reload` rotates with the config refresh. No-op when issuer is nil (degraded mode — Envoy ALS falls back to plaintext, CoreDNS OTel push stays cold) |
