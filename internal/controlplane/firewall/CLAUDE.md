...
| RPC | Scope | Purpose |
|-----|-------|---------|
| `FirewallInit` | global | Idempotent stack-up: `ensureConfigs` → ensure Envoy/CoreDNS images → ensure containers attached to `clawker-net` at static IPs → `WaitForHealthy`. Returns Envoy/CoreDNS IPs + network ID. BPF attach happens at CP startup, not here. Besides the RPC callers (`firewall up`, `controlplane up`, container-start bootstrap), the CP daemon itself invokes the handler method in-process as a pre-`SetReady` startup gate when `firewall.enable` (settings.yaml) is true (step 8c in `cmd/clawker-cp/main.go`; failure fails CP startup, exit code 1) — the ActionQueue serializes the two paths. |
