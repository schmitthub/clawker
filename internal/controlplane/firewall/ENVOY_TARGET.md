...
| ws | `tcpEgressLayer` | http upstream | `httpAppLayer` + `wsEnrich` (route upgrade_configs) |
| wss | `tlsSNIChainLayer` + `quicSNIChainLayer` | https upstream (cluster pinned http/1.1) | `httpAppLayer` + `wsEnrich` (route upgrade_configs) |
| ssh / tcp | `tcpDedicatedListener` | `tcpPinnedUpstream` | — (opaque `tcp_proxy`, no app) |
| tcp port-range | `tcpDedicatedListener` ×N (one per in-range port) | `tcpPinnedUpstream` ×N (LOGICAL_DNS host:port) | — |
| udp (raw) | `udpDedicatedListener` (UDP socket + `udp_proxy` listener filter) | `udpPinnedUpstream` (LOGICAL_DNS over UDP) | — (opaque datagrams, no app) |

Generation-wide facts decided once in the deriver (cannot be patched after a shared chain commits): the set of https/wss SNIs (→ one `server_names` chain + per-SNI cert each) + `tls_inspector` on the egress listener + the shared `http_egress_routes` route_config + the deny `default_filter_chain`; `httpDFPActive` / `httpsDFPActive`; the TCP listener-port assignment (`TCPPortBase + idx`, deterministic, must stay in lockstep with the eBPF `route_map`). NOTE: `require_sni` is **not** a fact we can use — it is unimplemented; the SNI gate is the per-SNI `server_names` chains + deny default, decided here.
...
2. **WS as a proto token — assumed yes.** `proto: ws|wss` enriches the http/https chain (per-route `upgrade_configs`, HCM `allow_connect:true`). Flag me if you'd rather it be a sub-attribute of an http rule.
...
- **Opaque `tcp` / `udp` / `ssh`** — `raw_tcp`/`raw_udp` goldens currently show empty configs (deriver skips them with a warning). Build the opaque dedicated-listener transports per the ssh/tcp/udp sections above (needs `buildTCPAccessLog`, dedicated-port assignment via `TCPMappings`/a new `UDPPortBase`).
- **`ws`/`wss`, port-range** — designed above, not built.
- **CIDR-range dst on a TLS-terminating proto** (`https`/`wss`) — latent gap. `certs.go` correctly skips minting a cert for a CIDR (no single host), but nothing rejects `dst: 10.0.0.0/24, proto: https` upstream, so it would emit a *certless* `STATIC` cluster whose `socket_address.address` is the CIDR string (invalid at runtime; `validateBootstrap` only checks it's decodeable). Belongs with the opaque-`tcp`/`udp` + port-range/CIDR pass — raw tcp/udp to a CIDR is the legitimate CIDR use-case; `https`-to-CIDR should fail closed there. Not in the representative ruleset today.
- **IP-over-h3 `use_original_dst` on the QUIC listener — UNVERIFIED.** `quicSNIChainLayer` emits a `prefix_ranges` + `use_original_dst` IP chain on the UDP/QUIC listener for atom completeness, but original-dst recovery / filter-chain matching on a QUIC listener is not grounded in Envoy source (docs are TCP-framed). Fail-closed if unsupported. Either ground it (QUIC listener original-dst support) or formally make the IP-https QUIC sibling no-emit (the one sanctioned per-dst-type carve-out, documented).
