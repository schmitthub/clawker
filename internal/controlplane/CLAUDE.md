...
9. Mark ready (`orchestrator.SetReady()`), serve `/healthz` on HealthPort. When `firewall.enable` (settings.yaml) is true, spawn the settings-driven firewall bringup goroutine (`settingsFirewallBringup` → in-process `FirewallInit`, queue-serialized with any CLI-triggered bringup) — the stack should be up whenever the CP is, including boots no CLI observes (restart policy). Async because first-boot image pull/build is unbounded and blocking `SetReady` would blow the CLI's `cpReadyTimeout` healthz wait; failure degrades with `event=firewall_stack_unavailable` (CP stays up, next `FirewallInit` retries)
