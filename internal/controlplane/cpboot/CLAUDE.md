...
| `bootstrap.go` | `EnsureRunning(ctx, EnsureOpts) (time.Duration, error)` (returns the host↔CP clock offset the readiness gate measured — the create path reuses it for the agent assertion; lifecycle callers ignore it) / `Stop(ctx, dc)` / `CPRunning(ctx, dc)` host-side lifecycle; `EnsureOpts` bundles `Docker` / `Config` / `Logger` / `HostDirs`. Drift gate: `cpBinaryHash` + `consts.LabelCPBinarySHA`. Image build: `cpImageDockerfile` recipe with content-derived tag (`cpImageRef`) and OCI provenance LABELs; `ensureCPImage` / `cpBuildContext`; `pruneStaleCPImages` post-build cleanup. Concurrent-bootstrap recovery: `recoverFromNameConflict` resolves Docker 409 via SHA match → image-creation-time ordering (`cpImageCreatedAt`) → retry sentinel `errCPRecoveryRetry`. Readiness gate: `cpReady` = `waitForCPHealthz` (+ `CPHealthTimeoutError`) then `waitForCPClockSync` (polls `adminclient.ProbeClockSkew`, + `newCPClockSyncTimeout`). |
...
var (
    ...
    probeSkewFn     = adminclient.ProbeClockSkew  // single GetSystemTime probe
)
```
...
Both `EnsureRunning` success exits (adopt-existing and freshly-created) return `cpReady(ctx, cfg)`, which runs `healthzFn` then `clockSyncFn` and propagates the offset `clockSyncFn` measured back out through `EnsureRunning`'s `time.Duration` return. The clock-sync step polls `adminclient.ProbeClockSkew` until `|skew| ≤ cpClockSkewTolerance` (2s) or `cpClockSyncTimeout` (30s) — a Docker Desktop VM clock that lagged during host sleep converges once NTP re-syncs, and CP is not considered "running" until host↔CP clocks align. The create path (`EnsureControlPlaneForCreate`) reuses this returned offset for the agent assertion instead of issuing its own `ProbeClockSkew`. This keeps the CLI from minting a Hydra agent assertion whose `iat` is in the CP's future (zero-leeway "token used before issued" → poisoned, non-re-mintable bootstrap material). On non-convergence `newCPClockSyncTimeout` returns an actionable error naming the lagging-VM cause and fix.
...
**Uses**: `internal/auth`, `internal/config`, `internal/consts`, `internal/controlplane/adminclient` (for `ProbeClockSkew` in the clock-sync gate), `internal/controlplane/firewall` (for `fwcp.EnvoyStackName` etc.), `internal/docker`, `internal/logger`, `pkg/whail`, `github.com/moby/moby/api/types/{container,mount,network}`.
