...
func (o *CPStartupOrchestrator) SetServiceProbes(cp config.ControlPlaneSettings, tlsCfg *tls.Config) {
	...
	o.probes = []serviceProbe{
		{name: "hydra-public", addr: fmt.Sprintf(consts.LoopbackIPv4+":%d", cp.HydraPublicPort), tls: true},
		{name: "hydra-admin", addr: fmt.Sprintf(consts.LoopbackIPv4+":%d", cp.HydraAdminPort), tls: true},
		{name: "kratos-public", addr: fmt.Sprintf(consts.LoopbackIPv4+":%d", cp.KratosPublicPort), tls: true},
		{name: "kratos-admin", addr: fmt.Sprintf(consts.LoopbackIPv4+":%d", cp.KratosAdminPort), tls: true},
		{name: "oathkeeper-proxy", addr: fmt.Sprintf(consts.LoopbackIPv4+":%d", cp.OathkeeperPort), tls: true},
		{name: "oathkeeper-api", addr: fmt.Sprintf(consts.LoopbackIPv4+":%d", cp.OathkeeperAPIPort), tls: true},
		{name: "grpc-admin", addr: fmt.Sprintf(consts.LoopbackIPv4+":%d", cp.AdminPort), tls: false},
	}
}
