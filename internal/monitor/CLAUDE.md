...
| `logs/in_untrusted` | `otlp` (no client auth) | `routing/untrusted` connector — only `service.name=claude-code` routes downstream; everything else is dropped |
...
> **Trusted block conditionality**: the `otlp/infra` receiver + all four trusted pipelines (`logs/in_trusted`, `logs/cp`, `logs/envoy`, `logs/coredns`) are rendered into `otel-config.yaml` only when `OtelInfraPort > 0`. When the port is zero (monitoring stack uninstalled or `otel_infra_port` cleared), the entire receiver block is omitted from collector config AND the senders gate on the same condition: Envoy ALS falls back to the stdout JSON sink (via `alsConfig` returning the empty struct), the CoreDNS otel plugin installs `noopEmitter` (no `CLAWKER_COREDNS_OTEL_ENDPOINT` env var), and the CP's mTLS log shipper stays cold. Pipelines and senders move together — there is no half-state where senders try to dial a missing receiver.
