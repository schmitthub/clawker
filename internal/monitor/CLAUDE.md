...
templates/opensearch-bootstrap/
  ...
  index-templates/
    ...
    clawker-netlogger.json             # eBPF egress event attributes (verdict + attribution + 4-tuple + domain) from netlogger (service.name=ebpf-networking); final_pipeline=envelope-normalize
  ism-policies/
...
| `logs/netlogger` *(trusted block, see note)* | `routing/trusted` (when `service.name=ebpf-networking` — stamped by netlogger's `*sdklog.LoggerProvider` via `controlplane.NewOtelLoggerProvider` over OTLP/gRPC + mTLS leaf from `otelcerts.LoadTLSConfig("netlogger")`); `resource/netlogger` stamps `ingest_source=netlogger` post-routing | `opensearch/logs_netlogger` (index `clawker-netlogger`), `debug` |
| `logs/trusted_unrouted` *(trusted block, see note)* | `routing/trusted` default branch (any `service.name` outside cp/envoy/coredns/ebpf-networking). Should never fire — only those four hold infra-intermediate-chained leaves today | `debug` only (operator can grep `docker logs clawker-otel-collector` to identify a misconfigured trusted sender) |
