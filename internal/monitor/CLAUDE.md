...
| `logs/coredns` | `filelog/coredns` (tails Docker JSON log files under `/var/lib/docker/containers`; filter operator keeps only lines beginning with `source=coredns`; `resource/coredns` stamps `service.name=coredns` + `ingest_source=coredns`) | `opensearch/logs_coredns` (index `clawker-coredns`), `debug` |
...
**CoreDNS query logs**: CoreDNS's `log` plugin writes each query as a logfmt line beginning with `source=coredns` (see `firewall/coredns_config.go::corefileLogFormat`). The collector's `filelog/coredns` receiver tails all Docker JSON log files (`/var/lib/docker/containers/*/*-json.log`, mounted RO into the collector), parses the json-file envelope, promotes the inner `log` field to body, then drops anything that doesn't start with the `source=coredns` sentinel. No CoreDNS binary or env change is needed — the prefix in the log format is the schema-stable filter key.
