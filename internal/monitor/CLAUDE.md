...
| `logs/in_trusted` *(trusted block, see note)* | `otlp/infra` (mTLS-gated; CLI root CA is `client_ca_file`) | `routing/trusted` connector — dispatches by sender-declared `service.name` |
...
> **Trusted block conditionality**: the `otlp/infra` receiver + all four trusted pipelines (`logs/in_trusted`, `logs/cp`, `logs/envoy`, `logs/coredns`) are rendered into `otel-config.yaml` only when the trusted infra lane is wired. When the infra issuer isn't available or cert minting fails, the entire receiver block is omitted from collector config AND the senders degrade to stdout-only: Envoy drops the OTel access-log sink + `otel_collector_als` cluster entirely (gated at the sender via `als.MTLS` in `buildHTTPAccessLog` / `buildTCPAccessLog` / `buildClusters`), keeping only the stdout JSON sink for `docker logs clawker-envoy` triage; the CoreDNS otel plugin installs `noopEmitter` (no `CLAWKER_COREDNS_OTEL_ENDPOINT` env var); the CP's mTLS log shipper stays cold. Infra services never push OTLP across the untrusted `otel-collector:4317` lane reserved for agent containers — the infra ingestion path is closed end-to-end at the sender, never spoofable noise on the untrusted receiver.
