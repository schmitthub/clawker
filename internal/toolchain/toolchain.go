// Package toolchain loads file-backed toolchain definitions — named,
...
// language toolchain (e.g. node = baked LTS install in root scope + nvm
...
// user config dir), user-registered (settings toolchains registry), and
...
// Definition is a loaded toolchain definition.
type Definition struct {
	// Name is the flat-namespace key requirers declare.
	Name string
	// Description is the manifest's human summary of what the toolchain
	// provisions.
	Description string
...
// manifest is the parsed toolchain.yaml.
type manifest struct {
	Description string `yaml:"description"`
}

// nameRe constrains a definition name: it is a registry key, a directory
// name, and a token in build.toolchains lists.
var nameRe = regexp.MustCompile(`^[a-zA-Z0-9][a-zA-Z0-9._-]{0,40}$`)

// ValidateName rejects names that cannot serve as toolchain registry keys.
func ValidateName(name string) error {
	if !nameRe.MatchString(name) {
		return fmt.Errorf("toolchain name %q is invalid (must match %s)", name, nameRe.String())
	}
	return nil
}

// Load reads a definition from fsys, whose root must be the definition
// directory (toolchain.yaml plus at least one fragment file). Use
...
func Load(name string, fsys fs.FS) (*Definition, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("toolchain %q: read %s: %w", name, ManifestFile, err)
	}
	var m manifest
	if unmarshalErr := yaml.Unmarshal(rawManifest, &m); unmarshalErr != nil {
		return nil, fmt.Errorf("toolchain %q: parse %s: %w", name, ManifestFile, unmarshalErr)
	}
	...
	if rootFragment == "" && userFragment == "" {
		return nil, fmt.Errorf(
			"toolchain %q: no fragment found — a definition ships %s, %s, or both",
...
func loadFragment(name string, fsys fs.FS, file string) (string, error) {
	...
	if err != nil {
		...
		return "", fmt.Errorf("toolchain %q: read %s: %w", name, file, err)
	}
	if strings.TrimSpace(string(raw)) == "" {
		return "", fmt.Errorf("toolchain %q: %s is empty", name, file)
	}
	if _, parseErr := template.New(name).Parse(string(raw)); parseErr != nil {
		return "", fmt.Errorf("toolchain %q: parse %s: %w", name, file, parseErr)
	}
