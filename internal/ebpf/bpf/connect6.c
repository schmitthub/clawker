...
// connect6.c — cgroup/connect6 program for IPv6 TCP deny.
//
// Blocks all non-loopback IPv6 TCP connections from managed containers.
// IPv6 bypasses Envoy (which binds IPv4 only on clawker-net), so allowing
// IPv6 TCP would let agents bypass the egress filter entirely.

#include "common.h"

// Check if an IPv6 address is ::1 (loopback).
