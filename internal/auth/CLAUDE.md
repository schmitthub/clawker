...
3. Waits for host↔CP clock sync before the first mint: loops a short-lived mTLS `GetSystemTime` probe (`probeCPTime`, no bearer token) until the CP's reported wall-clock is no longer behind the host (`!time.Now().After(cpTime)`), then mints the assertion in the **host clock** (the source of truth — Docker forces the CP/VM clock to track the host). Hydra/fosite validates `iat` with zero leeway, so the CLI *waits* for the (transient post-sleep) VM-clock lag to reconverge rather than shifting `iat` — once the CP clock is at/ahead of the host, a host-clock `iat` is already in the CP's past, so no backdate is needed. The wait is bounded by the eager-dial deadline (`initialTokenDeadline`, 15s) and latches once converged (`synced`), so a transient failure during CP bring-up self-heals on retry. On non-convergence the CLI **fails fast** — `token()` returns a wrapped "waiting for CP clock sync" error rather than degrading to a best-effort mint (which would only earn the "token used before issued" 500 the wait exists to avoid), so the operator sees a clear clock-sync cause instead of an opaque later 500.
