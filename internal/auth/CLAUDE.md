...
| `agent_cert.go` | `MintAgentCert(caCertPath, caKeyPath, project, agent)` returns an `AgentCert{CertPEM, KeyPEM, Thumbprint [32]byte}` — ephemeral 24h mTLS leaf signed by the CLI CA. The thumbprint is SHA-256 over the cert DER and is announced to the CP via `AdminService.AnnounceAgent` so any peer cert that doesn't match at `AgentService.Connect` is rejected (cert-swap defense). The CN is composed via `CanonicalAgentCN(project, agent)` so cert + ConnectRequest + slot key all line up on the composite identity. PEM material is returned for in-memory bootstrap delivery only; never persisted on the host. |
