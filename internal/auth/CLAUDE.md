...
| `identity.go` | Typed identity values `ProjectSlug` / `AgentName` with `New*` (wire-boundary validators) and `Must*` (test/migration paths) constructors; `AgentFullName(project, agent)` composer; `MaxShortNameLen()` reports the project/agent-name length cap (50 bytes — chosen with headroom for the docker volume-name budget, see `internal/docker/names_test.go::TestContainerName_HeadroomForMaxFields`). |
