...
Both attach and non-attach paths use `shared.CommandOpts` for DI. The `CommandOpts` wires: IOStreams, Config, Client, HostProxy, ControlPlane, AdminClient, SocketBridge, SocketGrants, Prompter, and Logger. `AgentName`/`Project` are left empty on both paths — an existing container's registry row already exists. Firewall eBPF programs are attached from outside the container by the eBPF manager, not by a container entrypoint.
