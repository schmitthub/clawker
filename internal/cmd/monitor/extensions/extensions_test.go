...
// The command inventories monitoring extensions only.
func TestMonitorExtensions_FloorExtensions(t *testing.T) {
	testenv.New(t)
	...
	cmd := extensions.NewCmdExtensions(f, nil)
	cmd.SetArgs([]string{})
	...
	// Monitoring extensions only — no stack rows.
