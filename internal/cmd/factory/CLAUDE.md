...
- `adminClientFunc(f)` -- returns lazy `adminv1.AdminServiceClient` constructor; closes over `f.Config()`, `f.Logger()`, `f.Client(ctx)`. On first call (or after `grpc.ClientConn` transitions to `TransientFailure`/`Shutdown`) it invokes `cpboot.EnsureRunning` directly, then dials via `adminclient.Dial(ctx, adminPort, hydraPort, grpc.WithKeepaliveParams(...))` (package `internal/controlplane/adminclient` composes `internal/auth` primitives with CP-specific topology). No test seams — callers that want to substitute behavior inject an `AdminServiceClient` mock at the Factory level (`cpmocks.AdminServiceClientMock`). No raw moby client.
