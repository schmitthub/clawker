...
- `adminClientFunc(f)` -- returns lazy `adminv1.AdminServiceClient` constructor; closes over `f.Config()`, `f.Logger()`, `f.Client(ctx)`. On first call (or after `grpc.ClientConn` transitions to `TransientFailure`/`Shutdown`) it invokes package-level `ensureRunning` (seam for `controlplane.EnsureRunning`), then dials via `adminclient.Dial(ctx, adminPort, hydraPort, grpc.WithKeepaliveParams(...))` (package `internal/controlplane/adminclient` composes `internal/auth` primitives with CP-specific topology). No raw moby client — the firewall exception is gone.
