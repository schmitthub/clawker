...
- `configFunc()` -- returns lazy `config.Config` gateway constructor (lazy-loads project + settings stores; the registry is touched only through `project.CurrentProjectRoot()` for the walk-up anchor). Resolves the project root at the call site via `project.CurrentProjectRoot()` and passes it to `config.NewConfig(config.WithProjectRoot(root))` to bound project-config walk-up (empty root → walk-up disabled)
