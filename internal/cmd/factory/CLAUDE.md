...
- `firewallFunc(f)` -- returns lazy `firewall.FirewallManager` constructor; closes over `f.Config()`, `f.Logger()`, and `f.Client(ctx)` to call `firewall.NewManager(client, cfg, log)`
