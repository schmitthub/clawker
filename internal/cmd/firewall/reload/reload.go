// Package reload provides the firewall reload command.
package reload
...
func reloadRun(ctx context.Context, opts *ReloadOptions) error {
	...
	resp, err := shared.CallWithSpinnerTimeout(ctx, ios, "Reloading firewall...",
	...
	if err != nil {
		...
		return shared.WrapRPCError("reloading firewall", err)
	}

	fmt.Fprintf(ios.Out, "%s Firewall configuration reloaded\n", cs.SuccessIcon())
	shared.PrintStackRestartedNote(ios, resp.GetStackRestarted(), "configuration regenerated")
	return nil
}
