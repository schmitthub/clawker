// Package status provides the firewall status command.
package status
...
//
//nolint:tagliatelle // snake_case keys are the published `firewall status --json` contract
...
func statusRun(ctx context.Context, opts *StatusOptions) error {
	...
	resp, err := shared.CallWithSpinner(ctx, opts.IOStreams, "Fetching firewall status...",
	...
	if err != nil {
		...
		return shared.WrapRPCError("getting firewall status", err)
	}
...
func renderStatus(opts *StatusOptions, row statusRow) error {
	...
	case opts.Format.IsTemplate():
		if err := cmdutil.ExecuteTemplate(
			ios.Out,
			opts.Format.Template(),
			cmdutil.ToAny([]statusRow{row}),
		); err != nil {
...
// printStatus writes the human-readable status block. Address lines are
// suppressed when the control plane reports no value for them.
func printStatus(ios *iostreams.IOStreams, row statusRow) {
	cs := ios.ColorScheme()
