// Package down provides the firewall down command.
package down
...
func downRun(ctx context.Context, opts *DownOptions) error {
	...
	if _, err = shared.CallWithSpinner(ctx, ios, "Stopping firewall stack...",
		...
		return shared.WrapRPCError("stopping firewall", err)
	}
