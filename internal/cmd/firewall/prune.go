...
func pruneRun(ctx context.Context, opts *PruneOptions) error {
	...
	if !opts.All {
		var err error
		keep, err = composeProjectRules(ctx, opts.Config, opts.ProjectManager)
		if err != nil {
			return err
		}
	}
	...
	resp, err := callWithSpinner(ctx, ios, "Removing all firewall rules...",
	...
	if err != nil {
		return wrapRPCError("pruning firewall rules", err)
	}
	...
	res, err := syncRules(ctx, ios, client, keep,
...
func reportWipe(opts *PruneOptions, resp *adminv1.FirewallRemoveRuleResult) error {
	...
	case adminv1.RemoveRuleStatus_REMOVE_RULE_STATUS_REMOVED:
		...
		if opts.All {
			printStackRestartedNote(ios, resp.GetStackRestarted(), "rules removed")
		}
