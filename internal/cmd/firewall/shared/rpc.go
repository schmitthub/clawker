// Package shared holds the helpers the firewall subcommand packages have in
// common: the spinner-wrapped AdminService RPC callers, remediation-aware
// error wrapping, the stack bringup UX, the config compose+sync pipeline, and
// flag validation. Subcommand packages import this; it never imports them.
package shared
...
// (status, list, add, remove, refresh, prune, enable, disable, bypass, down,
// rotate-ca). The CP queue serializes work so a single RPC can wait behind
// other queued actions + its own reconcile (worst case a few seconds). 15s
// gives comfortable headroom while keeping a stuck CP from hanging the CLI
// indefinitely.
//
...
//
//nolint:ireturn // T is the caller's concrete result type — generic passthrough, not an interface return
func CallWithSpinner[T any](
	ctx context.Context,
...
//
//nolint:ireturn // T is the caller's concrete result type — generic passthrough, not an interface return
func CallWithSpinnerTimeout[T any](
	ctx context.Context,
	ios *iostreams.IOStreams,
	label string,
