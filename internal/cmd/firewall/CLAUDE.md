...
| File | Purpose |
|------|---------|
| `firewall.go` | Parent command `NewCmdFirewall(f)` — registers all 13 subcommands |
...
| `prune.go` | `firewall prune` — wipe the rule store (`FirewallRemoveRule` with `all=true`), then re-sync the config-defined set (`--all` skips the re-sync); confirmation-gated (`--yes` to skip) |
| `sync.go` | Shared compose+sync helpers: `composeProjectRules` (firewall-enabled + current-project gates, then `bundler.EgressRules(cfg, "")` → proto) and `syncRules` (`FirewallAddRules` + per-status tally) — used by `refresh` and `prune` |
...
| `refresh` | `NewCmdRefresh(f, runF)` | none | none | `FirewallAddRules` (re-syncs `bundler.EgressRules(cfg, "")` → `adminv1.EgressRulesToProto`); global (no `--agent`); requires firewall enabled and a resolvable current project; add/update merge only (no prune — delete via `firewall remove` or reset via `firewall prune`) |
| `prune` | `NewCmdPrune(f, runF)` | none | `--all`/`-a` (remove everything, no re-sync — works outside a project, no firewall-enabled gate), `--yes`/`-y` (skip confirmation; required for approval in non-interactive sessions — the prompt otherwise resolves to its "no" default and aborts) | `FirewallRemoveRule` with `all=true` (one wipe mutation, one reconcile), then — unless `--all` — the same `FirewallAddRules` sync as `refresh`. The keep set is composed BEFORE the wipe so a config/harness failure never leaves an emptied store. `NOT_FOUND` on the wipe = empty store, not an error; the re-sync still runs |
