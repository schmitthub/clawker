...
func projectRegisterRun(ctx context.Context, opts *RegisterOptions) error {
	...
	//   2. clawker.yaml::project.name override
	...
	if projectName == "" {
		return fmt.Errorf("project name resolved to empty after normalization (raw input %q); set `project.name` in clawker.yaml or pass a name argument", raw)
	}
