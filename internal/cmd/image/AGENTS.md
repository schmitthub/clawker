...
- `image remove` / `image rm` / `image rmi` — remove specific images
...
Uses **live-display** output scenario: `BuildOptions` captures `IOStreams` and `TUI` from Factory plus lazy closures for `Config`, `Logger`, `Client`, `ProjectManager`, and `HttpClient`. Tags are harness-keyed, not arbitrary Docker tags: `harnessSelectorFromTags` derives the harness to build from `-t` (a bare NAME selects that harness; a `NAME:HARNESS` ref keeps riding as an extra image tag whose tag part must name a known harness — all `-t` entries must agree on one harness), resolved via `bundler.ResolveHarnessName`/`bundler.LoadHarness`; no `-t` builds the configured default harness and adds its `:default` alias tag. A shared per-project base image (`clawker-<project>:base`) is built or reused automatically before the harness image, which builds `FROM` it. Build progress is rendered via `opts.TUI.RunProgress(opts.Progress, cfg, ch)` — BubbleTea tree in TTY, plain text otherwise. BuildKit progress events flow through a `buildOpts.OnProgress` callback that forwards `whail.BuildProgressEvent` → `tui.ProgressStep` on a `chan tui.ProgressStep`. The builder runs in a goroutine; channel closure signals done. When `--quiet` or `--progress=none`, output is suppressed and `builder.Build` runs synchronously with no progress channel. Before building, the command calls `docker.BuildKitEnabled` and emits a warning if BuildKit is unavailable (cache mount directives are silently ignored in legacy mode). HttpClient is used to resolve the selected harness's version via `bundler.ResolveHarnessVersion` (per the harness manifest's version spec — npm dist-tag, GitHub release, or none), baked into the rendered Dockerfile's version ARG default. Resolution failure is non-fatal — a warning prints and `bundler.DefaultHarnessVersion` (`"latest"`) is used. IIDFile, when set, writes the built image digest to the named file after a successful build.
