...
- `HttpClient()` -- lazy `*http.Client` for outbound HTTP from the CLI (currently: npm registry lookups during Claude Code version resolution in `bundler.ResolveLatestClaudeCodeVersion`). Tests substitute by setting `f.HttpClient = func() (*http.Client, error) { return &http.Client{Transport: stubRoundTripper{}}, nil }` — `http.RoundTripper` is the stdlib mock seam (same shape as gh-CLI's `pkg/httpmock.Registry`). No project-defined interface; no test seam on production API.
