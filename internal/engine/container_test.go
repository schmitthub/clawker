...
func TestContainerName(t *testing.T) {
	tests := []struct {
		...
		{"my-project", "ralph", "clawker.my-project.ralph"},
		{"test", "worker", "clawker.test.worker"},
		{"project123", "clever-fox", "clawker.project123.clever-fox"},
	}
...
func TestVolumeName(t *testing.T) {
	tests := []struct {
		...
		{"my-project", "ralph", "workspace", "clawker.my-project.ralph-workspace"},
		{"my-project", "ralph", "config", "clawker.my-project.ralph-config"},
		{"test", "worker", "history", "clawker.test.worker-history"},
	}
...
func TestParseContainerName(t *testing.T) {
	tests := []struct {
		...
		{"clawker.myapp.ralph", "myapp", "ralph", true},
		{"clawker.test.worker", "test", "worker", true},
		{"/clawker.myapp.ralph", "myapp", "ralph", true}, // Docker adds leading /
		{"invalid-name", "", "", false},
		{"clawker-old-format", "", "", false},
		{"clawker.only-project", "", "", false},
	}
...
func TestVolumeNamingConsistency(t *testing.T) {
	...
	// All names should start with "clawker."
	if containerName[:8] != "clawker." {
		t.Errorf("Container name should start with 'clawker.': %s", containerName)
	}
	if workspaceVolume[:8] != "clawker." {
		t.Errorf("Workspace volume should start with 'clawker.': %s", workspaceVolume)
	}
	if configVolume[:8] != "clawker." {
		t.Errorf("Config volume should start with 'clawker.': %s", configVolume)
	}

	// Verify exact formats
	if containerName != "clawker.my-app.ralph" {
		t.Errorf("Container name format unexpected: %s", containerName)
	}
	if workspaceVolume != "clawker.my-app.ralph-workspace" {
		t.Errorf("Workspace volume format unexpected: %s", workspaceVolume)
	}
	if configVolume != "clawker.my-app.ralph-config" {
