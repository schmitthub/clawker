...
| `agentslots.Registry` | Short-lived registration slots reserved by `AdminService.AnnounceAgent` and consumed by `AgentService.Connect`. Composite key (`thumbprint, agent_name`) folds the agent_name cross-check into slot lookup. Constant-time PKCE compare; mismatched verifier preserves the slot (TTL evicts) so a benign clawkerd retry succeeds. Single-use Consume IS the replay defense. `EvictByContainerID` driven by `dockerevents.Subscribe` mirrors `agentregistry`'s eviction shape. Lives in `internal/controlplane/agentslots` |
| `agentregistry.Registry` | Tracks registered agents keyed by SHA-256 over the mTLS peer cert DER (`[sha256.Size]byte`). Channel-bound identity — `Lookup(thumbprint)` is the only path for per-agent RPCs to resolve a caller. `EvictByContainerID` invoked by the `dockerevents` subscription on container die/destroy. Lives in `internal/controlplane/agentregistry` |
| `agent.Handler` | `agentv1.AgentServiceServer.Connect` implementation (server-streaming) in `internal/controlplane/agent`. Cross-checks at Connect: cert CN vs request agent_name (constant-time, BEFORE slot consume), composite slot consume (folds thumbprint + agent_name into the lookup), peer IP vs Docker network position, label vs canonical agent name. Every failure returns one generic `codes.PermissionDenied` (no leak). Welcome sent BEFORE registry pin so Send-failure leaves no orphan; `<-ctx.Done()` idles for the agent's lifetime |
...
| `auth.MintAgentCert` | Generates a per-agent mTLS leaf cert signed by the CLI CA. CN = canonical agent name, 24h lifetime, returns PEM + lowercase-hex SHA-256 thumbprint over cert.Raw. Material is for tmpfs delivery only — never persisted on the host |
...
| `shared.AgentBootstrap` / `GenerateAgentBootstrap` / `WriteAgentBootstrapToContainer` / `AnnounceAgent` | Building blocks for the CLI's per-agent boot sequence: PKCE pair + mTLS leaf + CA + assertion in one struct, tar-streamed into the container's writable layer at `consts.BootstrapDir`, AnnounceAgent reserves the CP-side slot with the cert thumbprint + challenge |
...
| `cmd/clawkerd` | Per-container agent daemon. Reads bootstrap from `/run/clawker/bootstrap`, exchanges the CLI-signed assertion at Hydra → access token, mTLS-dials the CP agent listener, calls `AgentService.Connect` (server-streaming), receives Welcome (auth-success signal), deletes the single-use verifier, drains the command stream until SIGTERM or stream close. Bearer token attached via `PerRPCCredentials` (covers both unary and streaming RPCs) |
