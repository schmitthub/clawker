...
> **CP ≠ firewall.** Common LLM confusion. CP is unconditional infrastructure (auth, gRPC AdminService on AdminPort, AgentService listener on AgentPort, agentregistry bookkeeping, CP→clawkerd `agentdial` outbound dialer, mTLS, owns clawker-net). The firewall is one optional subsystem CP manages (Envoy + CoreDNS + eBPF egress enforcement), toggled by `firewall.enable` in `settings.yaml` (the master switch is global, NOT in `clawker.yaml` — the project schema's `security.firewall` holds per-project `add_domains`/`rules` only). Disabling firewall does NOT disable CP, agentregistry, agentdial→clawkerd Session, ListAgents, or any non-firewall AdminService RPC. Don't gate non-firewall behavior on the firewall flag.
...
| `agentregistry.Registry` | Sqlite-persisted record of registered agents keyed by SHA-256 over the mTLS peer cert DER (`[sha256.Size]byte`). CLI writes rows at container CREATE time; CP-side `Reap` (startup) + dockerevents `ContainerDestroyed`/`ContainerRemoved` (steady state) evict. Channel-bound identity — `Lookup(thumbprint, cn)` resolves an entry by thumbprint and verifies the supplied peer cert CN matches the pre-computed `canonical_cn` column. Mismatch on thumbprint OR CN collapses to `ErrUnknownAgent` so handlers can't distinguish which half failed. Backing store: `modernc.org/sqlite`, no in-memory cache. Lives in `internal/controlplane/agentregistry` |
...
| `agent.IdentityInterceptor` | Paired (unary, stream) interceptors in `internal/controlplane/agent/identity_interceptor.go`. Resolves peer cert thumbprint to an `agentregistry.Entry` for every non-opted-out RPC. Opt-out roster is empty in this branch (AgentService has no inbound RPCs). Stream wrapper's `Context()` is on the wrapper (NOT promoted) — promotion would silently break identity binding for streaming RPCs. Wired AFTER `AuthInterceptor` on the agent listener |
| `agentdial.Dialer` | CP-side outbound dialer for `ClawkerdService.Session`. Permissive trust by design: `InsecureSkipVerify=true` + `VerifyPeerCertificate` always returns nil. Every cert/CN/registry outcome surfaces as a typed `Provenance` field on the `SessionConnected` overseer event; the dial only fails on connectivity (TCP timeout, container gone, retry exhausted, ctx cancelled). fd-leak ceiling tracks Close failures; bounded ring buffer of panic times rate-limits subscribe-side circuit breaker. Lives in `internal/controlplane/agentdial` |
...
| `auth.CanonicalAgentCN` | Pure function `CanonicalAgentCN(project ProjectSlug, agent AgentName) string` composing the canonical agent CN. Typed identity values guarantee callers can't fabricate malformed components. Single source of truth for the canonical form, shared by `MintAgentCert` (issuance) and `agentregistry.Add` (pre-computed `canonical_cn` column read by `Lookup`). Empty project → 2-segment `clawker.<agent>` matching `docker.ContainerName` behavior |
...
| `cmd/clawkerd` | Per-container agent daemon. Reads bootstrap from `/run/clawker/bootstrap`, resolves `CLAWKER_AGENT` + `CLAWKER_PROJECT` env, starts the inbound `ClawkerdService` mTLS listener on `:7700` (CN-pinned to CP), idles on `ctx.Done` for the container's lifetime. Auth materials (assertion, verifier) are loaded from disk for future agent→CP RPCs but unused in this branch. No outbound dial. Structured zerolog via `internal/logger.New()` to `/var/log/clawker/clawkerd.log` (50MB rotation, 7d retain, 3 backups) |
