...
| `agentslots.Registry` | Short-lived registration slots reserved by `AdminService.AnnounceAgent` and consumed by `AgentService.Connect`. Composite key (`thumbprint, agent_name, project`) folds both halves of the wire identity into slot lookup — same short agent name across two projects keys disjoint slots. Constant-time PKCE compare; mismatched verifier preserves the slot (TTL evicts) so a benign clawkerd retry succeeds. Single-use Consume IS the replay defense. **TTL janitor is the sole correctness floor** — agentslots is intentionally NOT an Overseer subscriber; every failure path between Reserve and Consume collapses to "no Consume happens" and is handled by 60s TTL expiry. Lives in `internal/controlplane/agentslots` |
| `agentregistry.Registry` | Tracks registered agents keyed by SHA-256 over the mTLS peer cert DER (`[sha256.Size]byte`). Channel-bound identity — `Lookup(thumbprint, cn)` resolves an entry by thumbprint and verifies the supplied peer cert CN matches the stored `auth.CanonicalAgentCN(Project, AgentName)`. Mismatch on thumbprint OR CN collapses to `ErrUnknownAgent` so handlers can't distinguish which half failed. `EvictByContainerID` invoked by the Overseer subscription on `dockerevents.ContainerRemoved` (destroy only — stop/kill don't evict). Lives in `internal/controlplane/agentregistry` |
| `overseer.Overseer` | Typed event bus + in-memory worldview state for the CP. Replaces the deleted `informer` package. Producers (`dockerevents`, `agentdial`) call `overseer.Publish[T](bus, ev)` with typed events; subscribers call `Subscribe[T](bus, name)` for a typed channel. Bus loop owns subscriber registry + `State` (Containers + AgentSessions); `Snapshot(ctx)` returns a deep-copied projection. Per-subscriber drop-oldest under buffer pressure; panic recovery on filter/applier closures. Lives in `internal/controlplane/overseer` |
...
| `auth.CanonicalAgentCN` | Pure function `CanonicalAgentCN(project ProjectSlug, agent AgentName) string` composing the canonical agent CN. Typed identity values guarantee callers can't fabricate malformed components. Single source of truth for the canonical form, shared by `MintAgentCert`, `agent.Handler.Connect` (peer cert CN cross-check), and `agentregistry.Lookup` (CN equality check). Empty project → 2-segment `clawker.<agent>` matching `docker.ContainerName` behavior |
