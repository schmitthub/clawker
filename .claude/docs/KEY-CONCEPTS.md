...
| `agentregistry.Registry` | Sqlite-persisted record of registered agents keyed by SHA-256 over the mTLS peer cert DER (`[sha256.Size]byte`). CLI writes rows at container CREATE time; CP-side `Reap` (startup) + dockerevents `ContainerRemoved` (steady state) evict. Channel-bound identity — `Lookup(thumbprint, cn)` resolves an entry by thumbprint and verifies the supplied peer cert CN matches the pre-computed `canonical_cn` column. Mismatch on thumbprint OR CN collapses to `ErrUnknownAgent` so handlers can't distinguish which half failed. Backing store: `modernc.org/sqlite`, no in-memory cache. Lives in `internal/controlplane/agentregistry` |
