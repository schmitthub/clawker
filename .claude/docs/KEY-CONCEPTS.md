...
| `agentslots.Registry` | Short-lived registration slots reserved by `AdminService.AnnounceAgent` and consumed by `AgentService.Register`. Constant-time PKCE compare; mismatched verifier preserves the slot (TTL evicts) so a benign clawkerd retry succeeds. Single-use Consume IS the replay defense. Lives in `internal/controlplane/agentslots` |
...
| `agent.Handler` | `agentv1.AgentServiceServer.Register` implementation in `internal/controlplane/agent`. Five identity-binding cross-checks at Register: PKCE consume, cert thumbprint vs slot, peer IP vs Docker network position, label vs canonical agent name, slot consume atomic. Every failure returns one generic `codes.PermissionDenied` (no leak about which check failed) |
...
| `cmd/clawkerd` | Per-container agent daemon. Reads bootstrap from `/run/clawker/bootstrap`, exchanges the CLI-signed assertion at Hydra → access token, mTLS-dials the CP agent listener, calls `AgentService.Register`, deletes the verifier on success, idles on the open connection until SIGTERM |
| `controlplane.AgentMethodScopes()` | Per-listener scope vocabulary for the agent gRPC listener: `/clawker.agent.v1.AgentService/Register` → `agent:self:register`. Mirror of `AdminMethodScopes`; `TestAgentMethodScopes_CoversAllRPCs` walks the proto descriptor so a future RPC without a scope entry breaks the build |
