...
> **CP ≠ firewall.** Common LLM confusion. CP is unconditional infrastructure (auth, gRPC AdminService + AgentService, agent slot/registry bookkeeping, mTLS, owns clawker-net). The firewall is one optional subsystem CP manages (Envoy + CoreDNS + eBPF egress enforcement), toggled by `firewall.enable` in `settings.yaml` (the master switch is global, NOT in `clawker.yaml` — the project schema's `security.firewall` holds per-project `add_domains`/`rules` only). Disabling firewall does NOT disable CP, AnnounceAgent, clawkerd Register, ListAgents, or any non-firewall AdminService RPC. Don't gate non-firewall behavior on the firewall flag.
...
| `agentslots.Registry` | Short-lived registration slots reserved by `AdminService.AnnounceAgent` and consumed by `AgentService.Connect`. Composite key (`thumbprint, agent_name, project`) folds both halves of the wire identity into slot lookup — same short agent name across two projects keys disjoint slots. Constant-time PKCE compare; mismatched verifier preserves the slot (TTL evicts) so a benign clawkerd retry succeeds. Single-use Consume IS the replay defense. `EvictByContainerID` driven by `dockerevents.Subscribe` mirrors `agentregistry`'s eviction shape. Lives in `internal/controlplane/agentslots` |
| `agentregistry.Registry` | Tracks registered agents keyed by SHA-256 over the mTLS peer cert DER (`[sha256.Size]byte`). Channel-bound identity — `Lookup(thumbprint, cn)` resolves an entry by thumbprint and verifies the supplied peer cert CN matches the stored `auth.CanonicalAgentCN(Project, AgentName)`. Mismatch on thumbprint OR CN collapses to `ErrUnknownAgent` so handlers can't distinguish which half failed. `EvictByContainerID` invoked by the `dockerevents` subscription on container die/destroy. Lives in `internal/controlplane/agentregistry` |
| `agent.Handler` | `agentv1.AgentServiceServer.Connect` implementation (server-streaming) in `internal/controlplane/agent`. Cross-checks at Connect: cert CN vs `auth.CanonicalAgentCN(req.Project, req.AgentName)` (constant-time, BEFORE slot consume — defends wire-body project tampering), composite slot consume (`thumbprint + agent_name + project` folded into the lookup), peer IP vs Docker network position, BOTH `LabelAgent` AND `LabelProject` cross-check vs slot. Every failure returns one generic `codes.PermissionDenied` (no leak). Welcome sent BEFORE registry pin so Send-failure leaves no orphan; `<-ctx.Done()` idles for the agent's lifetime |
| `agent.IdentityInterceptor` | Paired (unary, stream) interceptors in `internal/controlplane/agent/identity_interceptor.go`. Resolves peer cert thumbprint to an `agentregistry.Entry` for every non-opted-out RPC. Connect is the sole opt-out (it authenticates itself). Stream wrapper's `Context()` is on the wrapper (NOT promoted) — promotion would silently break identity binding for streaming RPCs. Wired AFTER `AuthInterceptor` on the agent listener |
...
| `auth.MintAgentCert` | Generates a per-agent mTLS leaf cert signed by the CLI CA. Signature: `MintAgentCert(caCertPath, caKeyPath, project, agent string)`. CN is composed inside via `auth.CanonicalAgentCN(project, agent)` so every CLI caller produces the same canonical shape and the agent handler's CN cross-check has a single equality to enforce. 24h lifetime, returns PEM + lowercase-hex SHA-256 thumbprint over cert.Raw. Material is for tmpfs delivery only — never persisted on the host |
| `auth.CanonicalAgentCN` | Pure function `CanonicalAgentCN(project, agent string) string` composing the canonical agent CN. Single source of truth for the canonical form, shared by `MintAgentCert`, `agent.Handler.Connect` (peer cert CN cross-check), and `agentregistry.Lookup` (CN equality check). Empty project → 2-segment `clawker.<agent>` matching `docker.ContainerName` behavior |
...
| `shared.AgentBootstrap` / `GenerateAgentBootstrap` / `WriteAgentBootstrapToContainer` / `AnnounceAgent` | Building blocks for the CLI's per-agent boot sequence: PKCE pair + mTLS leaf + CA + assertion in one struct, tar-streamed into the container's writable layer at `consts.BootstrapDir`. `GenerateAgentBootstrap(caCert, caKey, project, agent, hydraURL, signingKey)` mints the cert with canonical CN composed from (project, agent). `AnnounceAgent(ctx, admin, b, project, agent, containerID)` reserves the CP-side slot with (project, agent) as separate wire fields, the cert thumbprint, and the PKCE challenge |
...
| `cmd/clawkerd` | Per-container agent daemon. Reads bootstrap from `/run/clawker/bootstrap`, resolves `CLAWKER_AGENT` + `CLAWKER_PROJECT` env, exchanges the CLI-signed assertion at Hydra → access token, mTLS-dials the CP agent listener, calls `AgentService.Connect({agent_name, project, code_verifier})` (server-streaming), receives Welcome (auth-success signal), deletes the single-use verifier, drains the command stream until SIGTERM or stream close. Structured zerolog via `internal/logger.New()` to `/var/log/clawkerd.log` (50MB rotation, 7d retain, 3 backups). Bearer token attached via `PerRPCCredentials` (covers both unary and streaming RPCs) |
| `controlplane.AgentMethodScopes()` | Per-listener scope vocabulary for the agent gRPC listener: maps both `Connect` (server-stream) and `Events` (client-stream stub) to `agent:self:register`. Mirror of `AdminMethodScopes`; `TestAgentMethodScopes_CoversAllRPCs` walks the proto descriptor so a future RPC without a scope entry breaks the build |
