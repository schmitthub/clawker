...
- `cmd/clawkerd` → `internal/clawkerd/assets/clawkerd` (embedded by `internal/clawkerd/embed.go` as `clawkerd.Binary`). The bundler streams it into every per-project agent build context (`internal/bundler/dockerfile.go`), so each generated `clawker-<project>:sha-<hash>` image carries it at `/usr/local/bin/clawkerd`. clawkerd is the container's `ENTRYPOINT` and runs as PID 1 — it supervises the user CMD (forks via `SysProcAttr.Credential` for kernel-side privilege drop, forwards signals to the child pgroup, two-phase Wait4 reaper for orphan drain). See `cmd/clawkerd/CLAUDE.md` for the full PID-1 contract. `clawkerd.Binary` is hashed in `bundler.EmbeddedScripts()` so a fresh clawkerd build invalidates the per-project image cache.
