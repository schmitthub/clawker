...
- `internal/controlplane/agentregistry` — sqlite-persisted registry keyed by SHA-256 cert thumbprint + container_id. CLI writes rows at container CREATE time; `Reap` (startup) + dockerevents `ContainerDestroyed` (steady state) evict.
