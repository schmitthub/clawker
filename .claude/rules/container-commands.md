...
- `shared.CreateContainer(ctx, opts)` is the single entry point for container creation — performs all init steps (workspace, config, env, create, inject). Inputs/outputs via `CreateContainerOptions`/`CreateContainerResult`; developer diagnostics go to zerolog and the caller owns all terminal output. Shared by `run` and `create`
...
- Three-phase command structure: Phase A (pre-progress: config+Docker+image+safety checks), Phase B (CreateContainer, typically run in a goroutine behind a spinner), Phase C (post-progress: warnings+output)
