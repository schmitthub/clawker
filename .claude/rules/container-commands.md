...
- `@` symbol triggers `client.ResolveImageWithSource(ctx, projectName)` for automatic image resolution; project name resolved via `project.ProjectManager.CurrentProject(ctx).Name()`
