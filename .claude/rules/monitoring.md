...
### netlogger (`service.name="ebpf-networking"`, index `clawker-netlogger`)
...
- Resource attribute `service.name=ebpf-networking` is stamped by the SDK Resource. `routing/trusted` dispatches to `logs/netlogger`; `resource/netlogger` stamps `ingest_source=netlogger` post-routing.
- Each record carries `event.name=ebpf.egress.flow` plus attributes `source=ebpf`, `verdict` (`allowed`/`denied`/`bypassed`), `container_id`, `agent`, `project`, `cgroup_id`, `bpf_ts_ns`, `dst_ip`, `dst_port`, `l4_proto` + `l4_proto_code`, `ipv6`, `ipv4_mapped`, `dst_host`, `domain_hash`. Every field is emitted on every record (zero/empty when unset) — operators filter at dashboard/query time.
...
- Trusted: `otlp/infra` receiver (mTLS, `client_ca_file` = **infra intermediate CA** — not the CLI root, which agents also hold) → `logs/in_trusted` (`memory_limiter` → `batch`) → `routing/trusted` connector (`error_mode: propagate`, `default_pipelines: [logs/trusted_unrouted]`) → dispatches by sender-declared `service.name` to `logs/cp` (`clawker-cp`), `logs/envoy` (`envoy`), `logs/coredns` (`coredns`), or `logs/netlogger` (`ebpf-networking`). Records with unmapped `service.name` land in `logs/trusted_unrouted` (debug-only — should never fire). mTLS is the auth boundary; `service.name` is honored, not overwritten.
