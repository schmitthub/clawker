...
**Routing topology**:
- Untrusted: `otlp` receiver (no client auth, plaintext) → `logs/in_untrusted` (`memory_limiter` → `resource/untrusted_otlp` stamps `ingest_source=untrusted_otlp` → `batch`) → `routing/untrusted` connector → `service.name=claude-code` reaches `logs/claude-code` and `service.name=clawker-cli` reaches `logs/clawker-cli`; everything else lands in `logs/untrusted_unrouted` (debug-only, collector stdout; not indexed — records are forgeable) via `default_pipelines`. Spoofed `service.name=envoy`/`coredns`/`clawker-cp` from this lane reaches no index. Metrics and traces on the untrusted lane go through dedicated pipelines (`metrics/untrusted`, `traces`) that also stamp `ingest_source=untrusted_otlp` so dashboards can separate forgeable sender-declared records from records anchored by mTLS handshake.
